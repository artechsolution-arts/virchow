...
if [ "$ENABLE_CRAFT" = "false" ] || [ "$ENABLE_CRAFT" = "False" ]; then
    echo "Virchow Craft is disabled (ENABLE_CRAFT=false), skipping template setup"
    exit 0
fi
...
CRAFT_BASE="/app/virchow/server/features/build/sandbox/kubernetes/docker"
...
echo "Setting up Virchow Craft templates..."
