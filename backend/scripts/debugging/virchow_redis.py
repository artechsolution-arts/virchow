...
from ee.virchow.server.tenants.user_mapping import get_tenant_id_for_email
...
from virchow.configs.app_configs import REDIS_DB_NUMBER
from virchow.configs.app_configs import REDIS_HOST
from virchow.configs.app_configs import REDIS_PASSWORD
from virchow.configs.app_configs import REDIS_PORT
from virchow.configs.app_configs import REDIS_SSL
...
from virchow.redis.redis_pool import RedisPool
...
class VirchowRedisCommand(Enum):
...
def virchow_redis(
    ...
    if command == VirchowRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == VirchowRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == VirchowRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == VirchowRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == VirchowRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == VirchowRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == VirchowRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == VirchowRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == VirchowRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == VirchowRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Virchow Redis Manager")
    parser.add_argument(
        "--command",
        type=VirchowRedisCommand,
        help="The command to run",
        choices=list(VirchowRedisCommand),
        required=True,
    )
