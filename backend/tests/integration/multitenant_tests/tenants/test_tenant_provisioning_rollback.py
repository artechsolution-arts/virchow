...
from ee.virchow.server.tenants.schema_management import create_schema_if_not_exists
from ee.virchow.server.tenants.schema_management import drop_schema
...
class TestTenantProvisioningRollback:
    ...
    def test_failed_provisioning_cleans_up_schema(self) -> None:
        ...
        """
        from ee.virchow.background.celery.tasks.tenant_provisioning.tasks import (
        ...
        with patch(
            "ee.virchow.background.celery.tasks.tenant_provisioning.tasks.setup_tenant"
        ) as mock_setup:
            mock_setup.side_effect = Exception("Simulated provisioning failure")

            with patch(
                "ee.virchow.background.celery.tasks.tenant_provisioning.tasks.create_schema_if_not_exists",
