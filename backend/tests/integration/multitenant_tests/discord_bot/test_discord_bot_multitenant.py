...
from virchow.db.discord_bot import register_guild
...
from virchow.server.manage.discord_bot.utils import REGISTRATION_KEY_PREFIX
...
class TestBotConfigIsolationCloudMode:
    ...
    def test_cannot_create_bot_config_in_cloud_mode(self) -> None:
        ...
        with patch("virchow.configs.app_configs.AUTH_TYPE", AuthType.CLOUD):
            from fastapi import HTTPException

            from virchow.server.manage.discord_bot.api import _check_bot_config_api_access
    ...
    def test_bot_token_from_env_only_in_cloud(self) -> None:
        ...
        from virchow.virchowbot.discord.utils import get_bot_token

        with (
            patch("virchow.virchowbot.discord.utils.DISCORD_BOT_TOKEN", "env_token"),
            patch("virchow.virchowbot.discord.utils.AUTH_TYPE", AuthType.CLOUD),
        ):
