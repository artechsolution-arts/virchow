...
# add virchow/backend to path (since this isn't done automatically when running as a script)
current_dir = Path(__file__).parent
virchow_dir = current_dir.parent.parent.parent.parent
sys.path.append(str(virchow_dir / "backend"))

# load env before app_config loads (since env doesn't get loaded when running as a script)
env_path = virchow_dir / ".vscode" / ".env"
...
from ee.virchow.server.query_and_chat.models import SearchFullResponse
from ee.virchow.server.query_and_chat.models import SendSearchQueryRequest
...
class SearchAnswerAnalyzer:
    ...
    def _perform_search(self, query: str) -> OneshotQAResult:
        """Perform a document search query against the Virchow API and time it."""
        ...
        try:
            ...
            if os.environ.get("VIRCHOW_API_KEY"):
                headers["Authorization"] = f"Bearer {os.environ.get('VIRCHOW_API_KEY')}"
...
def run_search_eval(
    ...
    # check virchow api key is set (auth is always required)
    if not os.environ.get("VIRCHOW_API_KEY"):
        raise RuntimeError(
            "VIRCHOW_API_KEY is required. Please create one in the admin panel and add it to the root .vscode/.env file."
        )

    # check virchow is running
    try:
        ...
        raise RuntimeError(f"Could not connect to Virchow API: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Base URL of the Virchow API server (default: %(default)s).",
    )
    parser.add_argument(
        "-s",
        "--search_only",
