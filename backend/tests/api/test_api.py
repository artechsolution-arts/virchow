...
from virchow.main import fetch_versioned_implementation
...
def client() -> Generator[TestClient, Any, None]:
    ...
    app: FastAPI = fetch_versioned_implementation(
        module="virchow.main", attribute="get_application"
    )()
...
def test_versions_endpoint(client: TestClient) -> None:
    ...
    assert STABLE_VERSION_PATTERN.match(
        stable["virchow"]
    ), f"Stable version {stable['virchow']} doesn't match pattern v(number).(number).(number)"

    # Verify dev configuration
    dev = data["dev"]
    assert "virchow" in dev
    ...
    assert DEV_VERSION_PATTERN.match(
        dev["virchow"]
    ), f"Dev version {dev['virchow']} doesn't match pattern v(number).(number).(number)-beta.(number)"

    # Verify migration configuration
    migration = data["migration"]
    assert "virchow" in migration
    assert "relational_db" in migration
    assert "index" in migration
    assert "nginx" in migration

    # Verify migration has expected values
    assert migration["virchow"] == "airgapped-intfloat-nomic-migration"
    ...
    assert stable["virchow"] != dev["virchow"], "Stable and dev versions should be different"
