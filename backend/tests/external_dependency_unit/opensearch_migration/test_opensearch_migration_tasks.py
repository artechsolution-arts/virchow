...
from virchow.background.celery.tasks.opensearch_migration.constants import (
    GET_VESPA_CHUNKS_SLICE_COUNT,
)
from virchow.background.celery.tasks.opensearch_migration.tasks import (
    is_continuation_token_done_for_all_slices,
)
from virchow.background.celery.tasks.opensearch_migration.tasks import (
    migrate_chunks_from_vespa_to_opensearch_task,
)
from virchow.background.celery.tasks.opensearch_migration.transformer import (
    transform_vespa_chunks_to_opensearch_chunks,
)
from virchow.configs.constants import PUBLIC_DOC_PAT
from virchow.configs.constants import SOURCE_TYPE
...
from virchow.db.models import Document
from virchow.db.models import OpenSearchDocumentMigrationRecord
from virchow.db.models import OpenSearchTenantMigrationRecord
from virchow.db.opensearch_migration import build_sanitized_to_original_doc_id_mapping
from virchow.db.search_settings import get_active_search_settings
from virchow.document_index.interfaces_new import TenantState
from virchow.document_index.opensearch.client import OpenSearchClient
from virchow.document_index.opensearch.client import OpenSearchIndexClient
from virchow.document_index.opensearch.client import wait_for_opensearch_with_timeout
from virchow.document_index.opensearch.constants import DEFAULT_MAX_CHUNK_SIZE
from virchow.document_index.opensearch.schema import DocumentChunk
from virchow.document_index.opensearch.search import DocumentQuery
from virchow.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
from virchow.document_index.vespa.vespa_document_index import VespaDocumentIndex
from virchow.document_index.vespa_constants import ACCESS_CONTROL_LIST
from virchow.document_index.vespa_constants import BLURB
from virchow.document_index.vespa_constants import BOOST
from virchow.document_index.vespa_constants import CHUNK_CONTEXT
from virchow.document_index.vespa_constants import CHUNK_ID
from virchow.document_index.vespa_constants import CONTENT
from virchow.document_index.vespa_constants import DOC_SUMMARY
from virchow.document_index.vespa_constants import DOC_UPDATED_AT
from virchow.document_index.vespa_constants import DOCUMENT_ID
from virchow.document_index.vespa_constants import DOCUMENT_SETS
from virchow.document_index.vespa_constants import EMBEDDINGS
from virchow.document_index.vespa_constants import FULL_CHUNK_EMBEDDING_KEY
from virchow.document_index.vespa_constants import HIDDEN
from virchow.document_index.vespa_constants import IMAGE_FILE_NAME
from virchow.document_index.vespa_constants import METADATA_LIST
from virchow.document_index.vespa_constants import METADATA_SUFFIX
from virchow.document_index.vespa_constants import PRIMARY_OWNERS
from virchow.document_index.vespa_constants import SECONDARY_OWNERS
from virchow.document_index.vespa_constants import SEMANTIC_IDENTIFIER
from virchow.document_index.vespa_constants import SOURCE_LINKS
from virchow.document_index.vespa_constants import TITLE
from virchow.document_index.vespa_constants import TITLE_EMBEDDING
from virchow.document_index.vespa_constants import USER_PROJECT
...
def full_deployment_setup() -> Generator[None, None, None]:
    ...
    """
    # Patch ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW just for this test because we
    # don't yet want that enabled for all tests.
    # TODO(andrei): Remove this once CI enables OpenSearch for all tests.
    with (
        patch(
            "virchow.configs.app_configs.ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW",
            True,
        ),
        patch("virchow.document_index.factory.ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW", True),
    ):
...
def patch_get_vespa_chunks_page_size() -> Generator[int, None, None]:
    ...
    with (
        patch(
            "virchow.background.celery.tasks.opensearch_migration.tasks.GET_VESPA_CHUNKS_PAGE_SIZE",
            test_page_size,
        ),
        patch(
            "virchow.background.celery.tasks.opensearch_migration.constants.GET_VESPA_CHUNKS_PAGE_SIZE",
            test_page_size,
        ),
    ):
...
@pytest.fixture(scope="function")
def enable_opensearch_indexing_for_virchow() -> Generator[None, None, None]:
    with patch(
        "virchow.background.celery.tasks.opensearch_migration.tasks.ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW",
        True,
    ):
        yield  # Test runs here.
...
@pytest.fixture(scope="function")
def disable_opensearch_indexing_for_virchow() -> Generator[None, None, None]:
    with patch(
        "virchow.background.celery.tasks.opensearch_migration.tasks.ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW",
        False,
    ):
...
class TestMigrateChunksFromVespaToOpenSearchTask:
    ...
    def test_chunk_migration_completes_successfully(
        ...
        enable_opensearch_indexing_for_virchow: None,  # noqa: ARG002
    ) -> None:
        """
    ...
    def test_chunk_migration_resumes_from_continuation_token(
        ...
        with patch(
            "virchow.background.celery.tasks.opensearch_migration.tasks.get_redis_client",
    ...
    def test_returns_none_when_feature_disabled(
        self,
        disable_opensearch_indexing_for_virchow: None,  # noqa: ARG002
    ) -> None:
