...
from ee.virchow.background.celery.tasks.external_group_syncing.tasks import (
    _perform_external_group_sync,
)
from ee.virchow.db.external_perm import ExternalUserGroup
from virchow.access.utils import build_ext_group_name_for_virchow
...
from virchow.db.models import PublicExternalUserGroup
from virchow.db.models import User__ExternalUserGroupId
...
class TestPerformExternalGroupSync:
    def test_initial_group_sync(self, db_session: Session) -> None:
        ...
        with patch(
            "ee.virchow.background.celery.tasks.external_group_syncing.tasks.get_source_perm_sync_config"
        ) as mock_config:
            # Mock sync config
            ...
            expected_group1_id = build_ext_group_name_for_virchow(
                "group1", DocumentSource.GOOGLE_DRIVE
            )
            expected_group2_id = build_ext_group_name_for_virchow(
                "group2", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group_id = build_ext_group_name_for_virchow(
    ...
    def test_mixed_regular_and_public_groups(self, db_session: Session) -> None:
        ...
        with patch(
            ...
            # Verify user groups
            user_groups = _get_user_external_groups(db_session, cc_pair.id)
            expected_regular_group_id = build_ext_group_name_for_virchow(
                "regular_group", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group1_id = build_ext_group_name_for_virchow(
            ...
            expected_public_group2_id = build_ext_group_name_for_virchow(
