...
from virchow.background.indexing.run_docfetching import connector_document_extraction
...
class TestDocprocessingPriorityInDocumentExtraction:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (False, VirchowCeleryPriority.HIGH),
            # Re-indexing (has last_successful_index_time) should get MEDIUM priority
            (True, VirchowCeleryPriority.MEDIUM),
        ],
    )
    @patch("virchow.background.indexing.run_docfetching.get_document_batch_storage")
    @patch("virchow.background.indexing.run_docfetching.MemoryTracer")
    @patch("virchow.background.indexing.run_docfetching._get_connector_runner")
    @patch(
        "virchow.background.indexing.run_docfetching.strip_null_characters",
        side_effect=lambda batch: batch,
    )
    @patch(
        "virchow.background.indexing.run_docfetching.get_recent_completed_attempts_for_cc_pair"
    )
    @patch(
        "virchow.background.indexing.run_docfetching.get_last_successful_attempt_poll_range_end"
    )
    @patch("virchow.background.indexing.run_docfetching.save_checkpoint")
    @patch("virchow.background.indexing.run_docfetching.get_latest_valid_checkpoint")
    @patch("virchow.background.indexing.run_docfetching.get_redis_client")
    @patch("virchow.background.indexing.run_docfetching.ensure_source_node_exists")
    @patch("virchow.background.indexing.run_docfetching.get_source_node_id_from_cache")
    @patch("virchow.background.indexing.run_docfetching.get_node_id_from_raw_id")
    @patch("virchow.background.indexing.run_docfetching.cache_hierarchy_nodes_batch")
    def test_docprocessing_priority_based_on_last_successful_index_time(
        ...
        expected_priority: VirchowCeleryPriority,
    ) -> None:
        """
