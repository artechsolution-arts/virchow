...
from virchow.virchowbot.discord.handle_commands import get_text_channels
from virchow.virchowbot.discord.handle_commands import handle_dm
from virchow.virchowbot.discord.handle_commands import handle_registration_command
from virchow.virchowbot.discord.handle_commands import handle_sync_channels_command
from virchow.virchowbot.discord.handle_message import process_chat_message
from virchow.virchowbot.discord.handle_message import send_error_response
from virchow.virchowbot.discord.handle_message import send_response
...
class TestGuildRegistrationCommand:
    ...
    async def test_register_guild_success(
        ...
        with (
            patch(
                "virchow.virchowbot.discord.handle_commands.parse_discord_registration_key",
                return_value="public",
            ),
            patch(
                "virchow.virchowbot.discord.handle_commands.get_session_with_tenant"
            ) as mock_session,
            patch(
                "virchow.virchowbot.discord.handle_commands.get_guild_config_by_registration_key"
            ) as mock_get_config,
            patch("virchow.virchowbot.discord.handle_commands.bulk_create_channel_configs"),
        ):
            mock_db = MagicMock()
...
class TestSyncChannelsCommand:
    ...
    async def test_sync_channels_adds_new(
        ...
        with (
            ...
            patch(
                "virchow.virchowbot.discord.handle_commands.get_guild_config_by_discord_id"
            ) as mock_get_guild,
            patch(
                "virchow.virchowbot.discord.handle_commands.get_guild_config_by_internal_id"
            ) as mock_get_guild_internal,
            patch(
                "virchow.virchowbot.discord.handle_commands.sync_channel_configs"
            ) as mock_sync,
        ):
            mock_db = MagicMock()
            mock_session.return_value.__enter__ = MagicMock(return_value=mock_db)
...
class TestBotLifecycle:
    ...
    async def test_setup_hook_initializes_cache(
        ...
        from virchow.virchowbot.discord.client import VirchowDiscordClient

        with (
            patch.object(
                VirchowDiscordClient,
                "__init__",
                lambda self: None,  # noqa: ARG005
            ),
            patch(
                "virchow.virchowbot.discord.client.DiscordCacheManager",
                return_value=mock_cache_manager,
            ),
            patch(
                "virchow.virchowbot.discord.client.VirchowAPIClient",
                return_value=mock_api_client,
            ),
        ):
            bot = VirchowDiscordClient()
    ...
    async def test_close_closes_api_client(
        ...
        with (
            ...
            patch.object(VirchowDiscordClient, "is_closed", return_value=True),
        ):
            bot = VirchowDiscordClient()
            bot.cache = mock_cache_manager
            bot.api_client = mock_api_client
