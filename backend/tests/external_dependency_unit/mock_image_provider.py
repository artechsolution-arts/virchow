...
from virchow.image_gen.interfaces import ImageGenerationProvider
...
def use_mock_image_generation_provider() -> (
    ...
    with (
        ...
        patch(
            "virchow.tools.tool_implementations.images.image_generation_tool.get_image_generation_provider",
            return_value=image_gen_provider,
        ),
        # Mock is_available to return True so the tool is registered
        patch(
            "virchow.tools.tool_implementations.images.image_generation_tool.ImageGenerationTool.is_available",
            return_value=True,
        ),
        # Mock the config lookup in tool_constructor to return a valid LLMConfig
        patch(
            "virchow.tools.tool_constructor._get_image_generation_config",
