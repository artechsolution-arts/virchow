...
from virchow.server.manage.llm.api import (
...
from virchow.server.manage.llm.models import TestLLMRequest as LLMTestRequest
...
class TestLLMConfigurationEndpoint:
    ...
    def test_successful_llm_test_with_new_provider(
        ...
        try:
            with patch(
                "virchow.server.manage.llm.api.test_llm", side_effect=mock_test_llm_success
            ):
    ...
    def test_failed_llm_test_raises_virchow_error(
        self,
        db_session: Session,
        provider_name: str,  # noqa: ARG002
    ) -> None:
        """
        Test that a failed LLM test raises an VirchowError with VALIDATION_ERROR.

        When test_llm returns an error message, the endpoint should raise
        an VirchowError with the error details.
        """
        ...
        try:
            with patch(
                "virchow.server.manage.llm.api.test_llm", side_effect=mock_test_llm_failure
            ):
                ...
                assert exc_info.value.error_code == VirchowErrorCode.VALIDATION_ERROR
    ...
    def test_uses_existing_provider_api_key_when_not_changed(
        ...
        try:
            ...
            with patch(
                "virchow.server.manage.llm.api.test_llm", side_effect=mock_test_llm_capture
            ):
