...
from virchow.file_store.file_store import S3BackedFileStore
...
class TestExternalStorageFileStore:
    ...
    def test_get_default_file_store_s3(self) -> None:
        ...
        with patch("virchow.configs.app_configs.FILE_STORE_BACKEND", "s3"):
    ...
    def test_s3_bucket_name_configuration(self) -> None:
        ...
        with patch(
            "virchow.file_store.file_store.S3_FILE_STORE_BUCKET_NAME", "my-test-bucket"
        ):
    ...
    def test_s3_key_generation_default_prefix(self) -> None:
        ...
        with (
            patch("virchow.file_store.file_store.S3_FILE_STORE_PREFIX", "virchow-files"),
            patch(
                "virchow.file_store.file_store.get_current_tenant_id",
            ...
            assert s3_key == "virchow-files/test-tenant/test-file.txt"

    def test_s3_key_generation_custom_prefix(self) -> None:
        """Test S3 key generation with custom prefix"""
        with (
            patch("virchow.file_store.file_store.S3_FILE_STORE_PREFIX", "custom-prefix"),
            patch(
                "virchow.file_store.file_store.get_current_tenant_id",
                return_value="test-tenant",
            ),
    ...
    def test_s3_key_generation_with_different_tenant_ids(self) -> None:
        ...
        with patch("virchow.file_store.file_store.S3_FILE_STORE_PREFIX", "virchow-files"):
            file_store = S3BackedFileStore(bucket_name="test-bucket")
            ...
            with patch(
                ...
                assert s3_key == "virchow-files/tenant-1/document.pdf"
            ...
            with patch(
                ...
                s3_key = file_store._get_s3_key("document.pdf")
                assert s3_key == "virchow-files/tenant-2/document.pdf"
            ...
            with patch(
                ...
                s3_key = file_store._get_s3_key("document.pdf")
                assert s3_key == "virchow-files/public/document.pdf"
    ...
    def test_s3_save_file_mock(
        ...
        with (
            patch(
                "virchow.file_store.file_store.S3_FILE_STORE_BUCKET_NAME", "test-bucket"
            ),
            patch("virchow.file_store.file_store.S3_FILE_STORE_PREFIX", "virchow-files"),
            patch("virchow.file_store.file_store.S3_AWS_ACCESS_KEY_ID", "test-key"),
            patch("virchow.file_store.file_store.S3_AWS_SECRET_ACCESS_KEY", "test-secret"),
        ):
            # Mock the database operation to avoid SQLAlchemy issues
            with patch("virchow.db.file_record.upsert_filerecord") as mock_upsert:
                ...
                assert call_args[1]["Key"] == "virchow-files/public/test-file.txt"
...
class TestFileStoreInterface:
    ...
    def test_file_store_postgres_when_configured(self) -> None:
        ...
        with patch("virchow.configs.app_configs.FILE_STORE_BACKEND", "postgres"):
            file_store = get_default_file_store()
