...
APP_CONFIGS_MODULE = "virchow.configs.app_configs"
FILE_READER_MODULE = "virchow.tools.tool_implementations.file_reader.file_reader_tool"
...
class TestSearchToolAvailability:
    ...
    @patch("virchow.db.connector.check_user_files_exist", return_value=True)
    @patch(
...
class TestValidateVectorDbKnowledge:
    @patch(
        "virchow.server.features.persona.api.DISABLE_VECTOR_DB",
        True,
    )
    def test_rejects_document_set_ids(self) -> None:
        from fastapi import HTTPException

        from virchow.server.features.persona.api import _validate_vector_db_knowledge
