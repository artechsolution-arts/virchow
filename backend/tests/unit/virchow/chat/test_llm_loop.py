...
from virchow.chat.llm_loop import _build_empty_llm_response_error
from virchow.chat.llm_loop import _try_fallback_tool_extraction
from virchow.chat.llm_loop import construct_message_history
    ...
    def test_extracts_xml_style_invoke_from_answer_when_required(self) -> None:
        llm_step_result = LlmStepResult(
            ...
            answer=(
                ...
                '["Virchow documentation", "Virchow docs", "Virchow platform"]'
        ...
        assert result.tool_calls[0].tool_args == {
            "queries": ["Virchow documentation", "Virchow docs", "Virchow platform"]
        }
    ...
    def test_extracts_xml_style_invoke_from_answer_when_auto(self) -> None:
        llm_step_result = LlmStepResult(
            ...
            raw_answer=(
                '<function_calls><invoke name="internal_search">'
                '<parameter name="queries" string="false">'
                '["Virchow documentation", "Virchow docs", "Virchow internal docs"]'
                "</parameter></invoke></function_calls>"
            ),
            tool_calls=None,
        )
        ...
        assert result.tool_calls[0].tool_name == "internal_search"
        assert result.tool_calls[0].tool_args == {
            "queries": ["Virchow documentation", "Virchow docs", "Virchow internal docs"]
        }
    ...
    def test_extracts_from_raw_answer_when_filtered_answer_has_no_xml(self) -> None:
        llm_step_result = LlmStepResult(
            ...
            raw_answer=(
                '<function_calls><invoke name="internal_search">'
                '<parameter name="queries" string="false">'
                '["Virchow documentation", "Virchow docs"]'
                "</parameter></invoke></function_calls>"
            ),
            tool_calls=None,
        )
        ...
        assert result.tool_calls[0].tool_name == "internal_search"
        assert result.tool_calls[0].tool_args == {
            "queries": ["Virchow documentation", "Virchow docs"]
        }
...
class TestEmptyLlmResponseClassification:
    ...
    def test_openai_empty_stream_is_classified_as_budget_exceeded(
        ...
        monkeypatch.setattr("virchow.chat.llm_loop.is_true_openai_model", lambda *_: True)
