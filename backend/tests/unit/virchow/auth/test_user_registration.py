...
from virchow.auth.users import UserManager
...
class TestDisposableEmailValidation:
    ...
    @pytest.mark.asyncio
    @patch("virchow.auth.users.is_disposable_email")
    @patch("virchow.auth.users.fetch_ee_implementation_or_noop")
    @patch("virchow.auth.users.get_async_session_context_manager")
    @patch("virchow.auth.users.get_user_count", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("virchow.auth.users.is_disposable_email")
    @patch("virchow.auth.users.verify_email_domain")
    @patch("virchow.auth.users.fetch_ee_implementation_or_noop")
    @patch("virchow.auth.users.get_async_session_context_manager")
    @patch("virchow.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("virchow.auth.users.MULTI_TENANT", False)
...
class TestMultiTenantInviteLogic:
    ...
    @patch("virchow.auth.users.SQLAlchemyUserAdminDB")
    @patch("virchow.auth.users.is_disposable_email", return_value=False)
    @patch("virchow.auth.users.verify_email_domain")
    @patch("virchow.auth.users.fetch_ee_implementation_or_noop")
    @patch("virchow.auth.users.get_async_session_context_manager")
    @patch("virchow.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("virchow.auth.users.verify_email_is_invited")
    @patch("virchow.auth.users.MULTI_TENANT", True)
    @patch("virchow.auth.users.CURRENT_TENANT_ID_CONTEXTVAR")
    @pytest.mark.asyncio
...
class TestSAMLOIDCBehavior:
    ...
    @patch("virchow.auth.users.get_invited_users")
    @patch("virchow.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("virchow.auth.users.AUTH_TYPE")
    def test_sso_bypasses_whitelist(
        ...
        from virchow.auth.users import verify_email_is_invited
        ...
        with patch("virchow.auth.users.AUTH_TYPE", auth_type):
            verify_email_is_invited("newuser@example.com")  # Should not raise

    @patch("virchow.auth.users.get_invited_users")
    @patch("virchow.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("virchow.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestWhitelistBehavior:
    ...
    @patch("virchow.auth.users.workspace_invite_only_enabled", return_value=False)
    @patch("virchow.auth.users.get_invited_users")
    @patch("virchow.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestSeatLimitEnforcement:
    ...
    def test_adding_user_fails_when_seats_full(self) -> None:
        ...
        with patch(
            "virchow.auth.users.fetch_ee_implementation_or_noop",
    ...
    def test_seat_limit_only_enforced_for_self_hosted(self) -> None:
        ...
        with patch("virchow.auth.users.MULTI_TENANT", True):
