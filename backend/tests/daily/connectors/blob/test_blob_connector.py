...
from virchow.connectors.blob.connector import BlobStorageConnector
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.S3, "virchow-connector-tests")], indirect=True
)
def test_blob_s3_connector(
    ...
    for doc in all_docs:
        ...
        if file_extension in VirchowFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
@pytest.mark.parametrize(
    "blob_connector",
    [(BlobType.R2, "virchow-daily-connector-tests", {"european_residency": True})],
    indirect=True,
)
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.GOOGLE_CLOUD_STORAGE, "virchow-test-1")], indirect=True
)
