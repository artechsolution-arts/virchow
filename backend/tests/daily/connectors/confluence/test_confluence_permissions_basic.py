...
from ee.virchow.external_permissions.confluence.doc_sync import confluence_doc_sync
from virchow.access.models import DocExternalAccess
...
from virchow.db.utils import DocumentRow
from virchow.db.utils import SortOrder
...
@patch("ee.virchow.external_permissions.confluence.doc_sync.VirchowDBCredentialsProvider")
@patch(
    "virchow.file_processing.extract_file_text.get_unstructured_api_key",
    return_value=None,
)
def test_confluence_connector_restriction_handling(
    ...
    # this prevents redis calls inside of VirchowConfluence
    ...
    non_restricted_emails = {"chris@virchow.app"}
    ...
    restricted_emails = {"chris@virchow.app", "hagen@danswer.ai", "oauth@virchow.app"}
    restricted_user_groups = {"confluence-admins-danswerai"}

    extra_restricted_emails = {"chris@virchow.app", "oauth@virchow.app"}
    extra_restricted_user_groups: set[str] = set()

    # note that this is only allowed since yuhong@virchow.app is a member of the
    # confluence-admins-danswerai group
    special_restricted_emails = {"chris@virchow.app", "yuhong@virchow.app", "oauth@virchow.app"}
