...
from virchow.cache.interface import CacheBackendType
from virchow.configs.constants import AuthType
from virchow.configs.constants import QueryHistoryType
from virchow.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from virchow.utils.logger import setup_logger

# Load environment variables from .env file if it exists
load_dotenv()

# Custom on-premises ingestion flags
USE_INVOICE_CHUNKER = os.environ.get("USE_INVOICE_CHUNKER", "false").lower() == "true"
USE_DOCLING_OCR = os.environ.get("USE_DOCLING_OCR", "false").lower() == "true"
...
ENTERPRISE_EDITION_ENABLED = (
    os.environ.get("ENABLE_PAID_ENTERPRISE_EDITION_FEATURES", "false").lower() == "true"
)
...
IMAGE_SUMMARIZATION_SYSTEM_PROMPT = os.environ.get(
    "IMAGE_SUMMARIZATION_SYSTEM_PROMPT", "You are an AI assistant tasked with summarizing images."
)
IMAGE_SUMMARIZATION_USER_PROMPT = os.environ.get(
    "IMAGE_SUMMARIZATION_USER_PROMPT", "Please summarize the contents of this image."
)
...
# 3. no queries
VIRCHOW_QUERY_HISTORY_TYPE = QueryHistoryType(
    (os.environ.get("VIRCHOW_QUERY_HISTORY_TYPE") or QueryHistoryType.NORMAL.value).lower()
)
...
# NOTE: if you are having problems accessing the Virchow web UI locally (especially
...
# and is available in Virchow EE
...
# restrict access to Virchow to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Virchow
...
SAML_CONF_DIR = os.environ.get("SAML_CONF_DIR") or "/app/virchow/configs/saml_config"
...
# If set, Virchow will listen to the `expires_at` returned by the identity
...
# of OpenSearch running for the relevant Virchow instance.
# NOTE: Now enabled on by default, unless the env indicates otherwise.
ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW = (
    os.environ.get("ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW", "true").lower() == "true"
)
...
ENABLE_OPENSEARCH_RETRIEVAL_FOR_VIRCHOW = (
    ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW
    and os.environ.get("ENABLE_OPENSEARCH_RETRIEVAL_FOR_VIRCHOW", "").lower() == "true"
)
...
# View the list here:
# https://github.com/virchow-dot-app/virchow/blob/main/backend/virchow/connectors/factory.py
...
CONFLUENCE_USE_VIRCHOW_USERS_FOR_GROUP_SYNC = (
    os.environ.get("CONFLUENCE_USE_VIRCHOW_USERS_FOR_GROUP_SYNC", "").lower() == "true"
)
...
# backend/virchow/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja.
...
# Logs Virchow only model interactions like prompts, responses, messages etc.
LOG_VIRCHOW_MODEL_INTERACTIONS = (
    os.environ.get("LOG_VIRCHOW_MODEL_INTERACTIONS", "").lower() == "true"
)
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "Virchow")
...
SCHEDULED_EVAL_PERMISSIONS_EMAIL = os.environ.get(
    "SCHEDULED_EVAL_PERMISSIONS_EMAIL", "roshan@virchow.app"
)
...
AUTO_LLM_CONFIG_URL = os.environ.get(
    "AUTO_LLM_CONFIG_URL",
    "https://raw.githubusercontent.com/virchow-dot-app/virchow/main/backend/virchow/llm/well_known_providers/recommended-models.json",
)
...
# founders@virchow.app OR message Chris Weaver or Yuhong Sun in the Virchow
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "virchow-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "virchow-files"
...
# These are Virchow-managed API keys provided to tenants by default
#####
...
STRIPE_PUBLISHABLE_KEY_URL = (
    "https://virchow-stripe-public.s3.amazonaws.com/publishable-key.txt"
)
