...
VIRCHOW_DEFAULT_APPLICATION_NAME = "Virchow"
VIRCHOW_DISCORD_URL = "https://discord.gg/4NA5SbzrWb"
VIRCHOW_UTM_SOURCE = "virchow_app"
VIRCHOW_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "virchow_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "virchow_anonymous_user"
...
VIRCHOW_METADATA_FILENAME = ".virchow_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of Virchow.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use Virchow as a search engine."
)
...
DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN = "virchowapikey.ai"
...
KV_SETTINGS_KEY = "virchow_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "virchow_enterprise_settings"
...
class DocumentSource(str, Enum):
    # Special case, document passed in via Virchow APIs without specifying a source type
...
class MessageType(str, Enum):
    ...
    USER_REMINDER = "user_reminder"  # Custom Virchow message type which is translated into a USER message when passed to the LLM
...
class VirchowRedisConstants:
    ACTIVE_FENCES = "active_fences"


class VirchowCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
VIRCHOW_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
VIRCHOW_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
VIRCHOW_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class VirchowCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{VIRCHOW_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{VIRCHOW_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{VIRCHOW_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{VIRCHOW_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{VIRCHOW_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
VIRCHOW_CELERY_BEAT_HEARTBEAT_KEY = "virchow:celery:beat:heartbeat"
...
class VirchowCallTypes(str, Enum):
    FIREFLIES = "FIREFLIES"
    GONG = "GONG"
