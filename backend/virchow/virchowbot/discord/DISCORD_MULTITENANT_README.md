...
The Discord bot uses a **single-client, multi-tenant** architecture where one `VirchowDiscordClient` instance serves multiple tenants (organizations) simultaneously. Tenant isolation is achieved through:
...
│                                                                     │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐    │
│  │   DiscordCacheManager   │    │      VirchowAPIClient          │    │
│  │                         │    │                             │    │
...
### 1. Cache Manager (`backend/virchow/virchowbot/discord/cache.py`)
...
### 2. API Client (`backend/virchow/virchowbot/discord/api_client.py`)

The `VirchowAPIClient` is a **stateless async HTTP client** that communicates with Virchow API pods.

#### Key Design: Per-Request API Key Injection

```python
class VirchowAPIClient:
...
```python
# backend/virchow/db/discord_bot.py
...
gated_tenants = fetch_ee_implementation_or_noop(
    "virchow.server.tenants.product_gating",
    "get_gated_tenants",
    set(),
)()
...
1. **Single Client, Multiple Tenants**: One `VirchowAPIClient` and one `DiscordCacheManager` instance serves all tenants via dynamic API key injection.
...
| Component | Path |
|-----------|------|
| Cache Manager | `backend/virchow/virchowbot/discord/cache.py` |
| API Client | `backend/virchow/virchowbot/discord/api_client.py` |
| Discord Client | `backend/virchow/virchowbot/discord/client.py` |
| API Key DB Operations | `backend/virchow/db/discord_bot.py` |
| Cache Manager Tests | `backend/tests/unit/virchow/virchowbot/discord/test_cache_manager.py` |
| API Client Tests | `backend/tests/unit/virchow/virchowbot/discord/test_api_client.py` |