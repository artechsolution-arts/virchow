"""Async HTTP client for communicating with Virchow API pods."""

import aiohttp

from virchow.chat.models import ChatFullResponse
from virchow.virchowbot.discord.constants import API_REQUEST_TIMEOUT
from virchow.virchowbot.discord.exceptions import APIConnectionError
from virchow.virchowbot.discord.exceptions import APIResponseError
from virchow.virchowbot.discord.exceptions import APITimeoutError
...
from virchow.utils.variable_functionality import build_api_server_url_for_http_requests

logger = setup_logger()


class VirchowAPIClient:
    """Async HTTP client for sending chat requests to Virchow API pods.

    This client manages an aiohttp session for making non-blocking HTTP
    requests to the Virchow API server. It handles authentication with per-tenant
    API keys and multi-tenant routing.

    Usage:
        client = VirchowAPIClient()
    ...
    def __init__(
        ...
        # TODO: Ideally, this override is only used when someone is launching an Virchow service independently
    ...
    async def send_chat_message(
        ...
        """Send a chat message to the Virchow API server and get a response.
