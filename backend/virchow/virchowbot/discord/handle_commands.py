...
from virchow.configs.app_configs import DISCORD_BOT_INVOKE_CHAR
from virchow.configs.constants import VIRCHOW_DISCORD_URL
from virchow.db.discord_bot import bulk_create_channel_configs
from virchow.db.discord_bot import get_guild_config_by_discord_id
from virchow.db.discord_bot import get_guild_config_by_internal_id
from virchow.db.discord_bot import get_guild_config_by_registration_key
from virchow.db.discord_bot import sync_channel_configs
from virchow.db.engine.sql_engine import get_session_with_tenant
from virchow.db.utils import DiscordChannelView
from virchow.virchowbot.discord.cache import DiscordCacheManager
from virchow.virchowbot.discord.constants import REGISTER_COMMAND
from virchow.virchowbot.discord.constants import SYNC_CHANNELS_COMMAND
from virchow.virchowbot.discord.exceptions import RegistrationError
from virchow.virchowbot.discord.exceptions import SyncChannelsError
from virchow.server.manage.discord_bot.utils import parse_discord_registration_key
...
async def handle_dm(message: discord.Message) -> None:
    ...
    dm_response = (
        ...
        f"[Virchow Discord]({VIRCHOW_DISCORD_URL}) for help!"
    )
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to Virchow. "
            "I'll respond to messages based on your server and channel settings set in Virchow."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            "This server is already registered.\n\nVirchowBot can only connect one Discord server to one Virchow workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the Virchow admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the Virchow admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact Virchow support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact Virchow support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the Virchow admin panel."
        )
