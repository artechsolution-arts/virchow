...
from virchow.evals.eval import run_eval
...
def run_remote(
    ...
    Args:
        base_url: Base URL of the remote server (e.g., "https://test.virchow.app")
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        type=str,
        default="https://test.virchow.app",
        help="Base URL of the remote server (default: https://test.virchow.app)",
    )
    ...
    if args.remote:
        if not args.api_key:
            print("Using API Key from VIRCHOW_EVAL_API_KEY")
        api_key: str = (
            args.api_key if args.api_key else os.environ.get("VIRCHOW_EVAL_API_KEY", "")
        )
