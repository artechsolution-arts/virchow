...
from virchow.configs.app_configs import ANTHROPIC_DEFAULT_API_KEY
from virchow.configs.app_configs import COHERE_DEFAULT_API_KEY
from virchow.configs.app_configs import OPENAI_DEFAULT_API_KEY
from virchow.configs.app_configs import OPENROUTER_DEFAULT_API_KEY
from virchow.db.usage import check_usage_limit
from virchow.db.usage import UsageLimitExceededError
from virchow.db.usage import UsageType
from virchow.server.tenant_usage_limits import TenantUsageLimitKeys
from virchow.server.tenant_usage_limits import TenantUsageLimitOverrides
...
# Collect all Virchow-managed default API keys for comparison
_VIRCHOW_MANAGED_API_KEYS: set[str] = set()
for key in [
    ...
    if key:
        _VIRCHOW_MANAGED_API_KEYS.add(key)


def is_virchow_managed_api_key(api_key: str | None) -> bool:
    """Check if the given API key is one of Virchow's managed default keys."""
    return bool(api_key) and api_key in _VIRCHOW_MANAGED_API_KEYS
...
def is_tenant_on_trial_fn(tenant_id: str) -> bool:
    ...
    fn: Callable[[str], bool] = fetch_versioned_implementation(
        "virchow.server.usage_limits", "is_tenant_on_trial"
    )
    return fn(tenant_id)
...
def _get_tenant_override(tenant_id: str, field_name: str) -> int | None:
    ...
    try:
        ...
        get_overrides_fn = fetch_versioned_implementation(
            "virchow.server.tenant_usage_limits", "get_tenant_usage_limit_overrides"
        )
...
def check_llm_cost_limit_for_provider(
    ...
    """
    Check if the LLM cost limit would be exceeded for a provider using Virchow-managed keys.

    Only enforces limits when the provider uses Virchow-managed API keys.
    ...
    # Only enforce limits for Virchow-managed API keys
    if not is_virchow_managed_api_key(llm_provider_api_key):
        return
