...
from virchow.configs.app_configs import DISABLE_USER_KNOWLEDGE
from virchow.configs.app_configs import ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW
from virchow.configs.app_configs import VIRCHOW_QUERY_HISTORY_TYPE
from virchow.configs.app_configs import SHOW_EXTRA_CONNECTORS
from virchow.configs.app_configs import USER_FILE_MAX_UPLOAD_SIZE_MB
from virchow.configs.constants import KV_SETTINGS_KEY
...
def load_settings() -> Settings:
    ...
    try:
        ...
        if value is not None:
            ...
            cache.set(VirchowRedisLocks.ANONYMOUS_USER_ENABLED, "0", ex=SETTINGS_TTL)
    except Exception as e:
        logger.error(f"Error loading anonymous user setting from cache: {str(e)}")
        anonymous_user_enabled = False

    settings.anonymous_user_enabled = anonymous_user_enabled
    settings.query_history_type = VIRCHOW_QUERY_HISTORY_TYPE
    ...
    settings.opensearch_indexing_enabled = ENABLE_OPENSEARCH_INDEXING_FOR_VIRCHOW
    return settings
...
def store_settings(settings: Settings) -> None:
    ...
    if settings.anonymous_user_enabled is not None:
        cache.set(
            VirchowRedisLocks.ANONYMOUS_USER_ENABLED,
