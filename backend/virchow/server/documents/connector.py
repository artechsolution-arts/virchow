...
from virchow.background.celery.tasks.pruning.tasks import (
    try_creating_prune_generator_task,
)
from virchow.background.celery.versioned_apps.client import app as client_app
from virchow.configs.app_configs import EMAIL_CONFIGURED
from virchow.configs.app_configs import ENABLED_CONNECTOR_TYPES
from virchow.configs.app_configs import MOCK_CONNECTOR_FILE_PATH
from virchow.configs.constants import DocumentSource
from virchow.configs.constants import FileOrigin
from virchow.configs.constants import MilestoneRecordType
from virchow.configs.constants import VIRCHOW_METADATA_FILENAME
...
from virchow.connectors.factory import validate_ccpair_for_user
from virchow.connectors.google_utils.google_auth import (
    get_google_oauth_creds,
)
from virchow.connectors.google_utils.google_kv import (
...
from virchow.connectors.google_utils.google_kv import get_auth_url
from virchow.connectors.google_utils.google_kv import (
    get_google_app_cred,
)
from virchow.connectors.google_utils.google_kv import (
...
from virchow.connectors.google_utils.google_kv import verify_csrf
from virchow.connectors.google_utils.shared_constants import DB_CREDENTIALS_DICT_TOKEN_KEY
from virchow.connectors.google_utils.shared_constants import (
    GoogleOAuthAuthenticationMethod,
)
from virchow.db.connector import create_connector
from virchow.db.connector import delete_connector
from virchow.db.connector import fetch_connector_by_id
from virchow.db.connector import fetch_connectors
from virchow.db.connector import fetch_unique_document_sources
from virchow.db.connector import get_connector_credential_ids
from virchow.db.connector import mark_ccpair_with_indexing_trigger
from virchow.db.connector import update_connector
from virchow.db.connector_credential_pair import add_credential_to_connector
from virchow.db.connector_credential_pair import (
    fetch_connector_credential_pair_for_connector,
)
from virchow.db.connector_credential_pair import get_cc_pair_groups_for_ids
from virchow.db.connector_credential_pair import get_connector_credential_pair
from virchow.db.connector_credential_pair import get_connector_credential_pairs_for_user
from virchow.db.connector_credential_pair import (
    get_connector_credential_pairs_for_user_parallel,
)
from virchow.db.connector_credential_pair import verify_user_has_access_to_cc_pair
from virchow.db.credentials import cleanup_gmail_credentials
from virchow.db.credentials import cleanup_google_drive_credentials
from virchow.db.credentials import create_credential
from virchow.db.credentials import delete_service_account_credentials
from virchow.db.credentials import fetch_credential_by_id_for_user
from virchow.db.deletion_attempt import check_deletion_attempt_is_allowed
from virchow.db.document import get_document_counts_for_all_cc_pairs
from virchow.db.engine.sql_engine import get_session
from virchow.db.enums import AccessType
from virchow.db.enums import ConnectorCredentialPairStatus
from virchow.db.enums import IndexingMode
from virchow.db.enums import ProcessingMode
from virchow.db.federated import fetch_all_federated_connectors_parallel
from virchow.db.index_attempt import get_index_attempts_for_cc_pair
from virchow.db.index_attempt import get_latest_index_attempts_by_status
from virchow.db.index_attempt import get_latest_index_attempts_parallel
from virchow.db.models import ConnectorCredentialPair
from virchow.db.models import FederatedConnector
from virchow.db.models import IndexAttempt
from virchow.db.models import IndexingStatus
from virchow.db.models import User
from virchow.db.models import UserRole
from virchow.file_processing.file_types import PLAIN_TEXT_MIME_TYPE
from virchow.file_processing.file_types import WORD_PROCESSING_MIME_TYPE
from virchow.file_store.file_store import FileStore
...
from virchow.server.documents.models import AuthStatus
from virchow.server.documents.models import AuthUrl
from virchow.server.documents.models import ConnectorBase
from virchow.server.documents.models import ConnectorCredentialPairIdentifier
from virchow.server.documents.models import ConnectorFileInfo
from virchow.server.documents.models import ConnectorFilesResponse
from virchow.server.documents.models import ConnectorIndexingStatusLite
from virchow.server.documents.models import ConnectorIndexingStatusLiteResponse
from virchow.server.documents.models import ConnectorRequestSubmission
from virchow.server.documents.models import ConnectorSnapshot
from virchow.server.documents.models import ConnectorStatus
from virchow.server.documents.models import ConnectorUpdateRequest
from virchow.server.documents.models import CredentialBase
from virchow.server.documents.models import CredentialSnapshot
from virchow.server.documents.models import DocsCountOperator
from virchow.server.documents.models import FailedConnectorIndexingStatus
from virchow.server.documents.models import FileUploadResponse
from virchow.server.documents.models import GDriveCallback
from virchow.server.documents.models import GmailCallback
from virchow.server.documents.models import GoogleAppCredentials
from virchow.server.documents.models import GoogleServiceAccountCredentialRequest
from virchow.server.documents.models import GoogleServiceAccountKey
from virchow.server.documents.models import IndexedSourcesResponse
from virchow.server.documents.models import IndexingStatusRequest
from virchow.server.documents.models import ObjectCreationIdResponse
from virchow.server.documents.models import RunConnectorRequest
from virchow.server.documents.models import SourceSummary
from virchow.server.federated.models import FederatedConnectorStatus
from virchow.server.models import StatusResponse
from virchow.server.utils_vector_db import require_vector_db
from virchow.utils.logger import setup_logger
from virchow.utils.telemetry import mt_cloud_telemetry
from virchow.utils.threadpool_concurrency import CallableProtocol
from virchow.utils.threadpool_concurrency import run_functions_tuples_in_parallel
from virchow.utils.variable_functionality import fetch_ee_implementation_or_noop
...
def save_zip_metadata_to_file_store(
    ...
    """
    Extract .virchow_metadata.json from zip and save to file store.
    Returns the file_id or None if no metadata file exists.
    """
    try:
        metadata_file_info = zf.getinfo(VIRCHOW_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            ...
            try:
                ...
                logger.warning(f"Unable to load {VIRCHOW_METADATA_FILENAME}: {e}")
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {VIRCHOW_METADATA_FILENAME}: {e}",
                )

            # Save to file store
            file_id = file_store.save_file(
                content=BytesIO(metadata_bytes),
                display_name=VIRCHOW_METADATA_FILENAME,
                file_origin=FileOrigin.CONNECTOR_METADATA,
                file_type="application/json",
            )
            return file_id
    except KeyError:
        logger.info(f"No {VIRCHOW_METADATA_FILENAME} file")
        return None
    ...
    try:
        ...
        for file in files:
            ...
            if file.content_type == WORD_PROCESSING_MIME_TYPE:
                ...
                from virchow.file_processing.extract_file_text import read_docx_file
    ...
    try:
        ...
        if new_file_paths:
            ...
            client_app.send_task(
                VirchowCeleryTask.CHECK_FOR_INDEXING,
...
def create_connector_from_model(
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "virchow.db.user_group", "validate_object_creation_for_user", None
        )(
            db_session=db_session,
            user=user,
...
def submit_connector_request(
    ...
    Tracks via PostHog telemetry and sends email to hello@virchow.app.
    """
    tenant_id = get_current_tenant_id()
    ...
    if EMAIL_CONFIGURED:
        try:
            subject = "Virchow Craft Connector Request"
...
<li><strong>Tenant ID:</strong> {tenant_id}</li>
            ...
            send_email(
                user_email="hello@virchow.app",
                subject=subject,
                html_body=email_body_html,
                text_body=email_body_text,
            )
            logger.info(
                f"Connector request email sent to hello@virchow.app for connector: {connector_name}"
            )
        except Exception as e:
