...
from virchow.auth.anonymous_user import fetch_anonymous_user_info
from virchow.auth.email_utils import send_user_email_invite
from virchow.auth.invited_users import get_invited_users
from virchow.auth.invited_users import remove_user_from_invited_users
from virchow.auth.invited_users import write_invited_users
from virchow.auth.schemas import UserRole, UserCreate, UserStatusEnum
from virchow.auth.users import anonymous_user_enabled
...
from virchow.auth.users import enforce_seat_limit
from virchow.auth.users import optional_user
from virchow.auth.users import get_user_manager
from virchow.configs.app_configs import AUTH_BACKEND
from virchow.configs.app_configs import AUTH_TYPE
from virchow.configs.app_configs import AuthBackend
from virchow.configs.app_configs import DEV_MODE
from virchow.configs.app_configs import EMAIL_CONFIGURED
from virchow.configs.app_configs import ENABLE_EMAIL_INVITES
from virchow.configs.app_configs import NUM_FREE_TRIAL_USER_INVITES
from virchow.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from virchow.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from virchow.configs.app_configs import USER_AUTH_SECRET
from virchow.configs.app_configs import VALID_EMAIL_DOMAINS
from virchow.configs.constants import FASTAPI_USERS_AUTH_COOKIE_NAME
from virchow.configs.constants import PUBLIC_API_TAGS
from virchow.db.api_key import is_api_key_email_address
from virchow.db.auth import get_live_users_count
from virchow.db.engine.sql_engine import get_session
from virchow.db.enums import UserFileStatus
from virchow.db.models import User
from virchow.db.models import UserFile
from virchow.db.user_preferences import activate_user
from virchow.db.user_preferences import deactivate_user
from virchow.db.user_preferences import get_all_user_assistant_specific_configs
from virchow.db.user_preferences import get_latest_access_token_for_user
from virchow.db.user_preferences import update_assistant_preferences
from virchow.db.user_preferences import update_user_assistant_visibility
from virchow.db.user_preferences import update_user_auto_scroll
from virchow.db.user_preferences import update_user_chat_background
from virchow.db.user_preferences import update_user_default_app_mode
from virchow.db.user_preferences import update_user_default_model
from virchow.db.user_preferences import update_user_personalization
from virchow.db.user_preferences import update_user_pinned_assistants
from virchow.db.user_preferences import update_user_role
from virchow.db.user_preferences import update_user_shortcut_enabled
from virchow.db.user_preferences import update_user_temperature_override_enabled
from virchow.db.user_preferences import update_user_theme_preference
from virchow.db.users import batch_get_user_groups
from virchow.db.users import delete_user_from_db
from virchow.db.users import get_all_accepted_users
from virchow.db.users import get_all_users
from virchow.db.users import get_page_of_filtered_users
from virchow.db.users import get_total_filtered_users_count
from virchow.db.users import get_user_by_email
from virchow.db.users import get_user_counts_by_role_and_status
from virchow.db.users import validate_user_role_update
from virchow.key_value_store.factory import get_kv_store
from virchow.redis.redis_pool import get_raw_redis_client
from virchow.server.documents.models import PaginatedReturn
from virchow.server.features.projects.models import UserFileSnapshot
from virchow.server.manage.models import AllUsersResponse
from virchow.server.manage.models import AutoScrollRequest
from virchow.server.manage.models import BulkInviteResponse
from virchow.server.manage.models import ChatBackgroundRequest
from virchow.server.manage.models import DefaultAppModeRequest
from virchow.server.manage.models import EmailInviteStatus
from virchow.server.manage.models import MemoryItem
from virchow.server.manage.models import PersonalizationUpdateRequest
from virchow.server.manage.models import TenantInfo
from virchow.server.manage.models import TenantSnapshot
from virchow.server.manage.models import ThemePreferenceRequest
from virchow.server.manage.models import UserByEmail
from virchow.server.manage.models import UserInfo
from virchow.server.manage.models import UserPreferences
from virchow.server.manage.models import UserRoleResponse
from virchow.server.manage.models import UserRoleUpdateRequest
from virchow.server.manage.models import UserSpecificAssistantPreference
from virchow.server.manage.models import UserSpecificAssistantPreferences
from virchow.server.models import FullUserSnapshot
from virchow.server.models import InvitedUserSnapshot
from virchow.server.models import MinimalUserSnapshot
from virchow.server.models import UserGroupInfo
from virchow.server.usage_limits import is_tenant_on_trial_fn
from virchow.server.utils import BasicAuthenticationError
...
@router.post("/manage/admin/users/add", tags=PUBLIC_API_TAGS)
async def add_user(
    ...
    existing_user = get_user_by_email(email=user_create.email, db_session=db_session)
    if existing_user:
        raise HTTPException(status_code=400, detail="User already exists")

    # user_manager.create uses the async DB session (fastapi-users pattern).
    # After creation we re-fetch via the sync session so that eager-loaded
    # relationships (oauth_accounts) are available for FullUserSnapshot.
    await user_manager.create(user_create)

    created_user = get_user_by_email(email=user_create.email, db_session=db_session)
    if not created_user:
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "virchow.server.saml", "upsert_saml_user", None
    )(email=request.email)
...
def list_accepted_users(
    ...
    try:
        from virchow.db.models import ScimUserMapping
...
def list_all_users(
    ...
    accepted_users = [user for user in users]

    accepted_emails = {user.email for user in accepted_users}
    invited_emails = get_invited_users()

    # Filter out users who are already active
    all_active_emails = accepted_emails
    invited_emails = [
...
def bulk_invite_users(
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "virchow.server.tenants.provisioning", "add_users_to_tenant", None
    ...
    if MULTI_TENANT and not DEV_MODE:
        ...
        try:
            ...
            fetch_ee_implementation_or_noop(
                "virchow.server.tenants.billing", "register_tenant_users", None
            ...
            fetch_ee_implementation_or_noop(
                "virchow.server.tenants.user_mapping", "remove_users_from_tenant", None
            )(new_invited_emails, tenant_id)
            raise e
...
def deactivate_user_api(
    ...
    if not MULTI_TENANT:
        fetch_ee_implementation_or_noop(
            "virchow.db.license", "invalidate_license_cache", None
        )()
...
def verify_user_logged_in(
    ...
    team_name = fetch_ee_implementation_or_noop(
        "virchow.server.tenants.user_mapping", "get_tenant_id_for_email", None
    )(user.email)
    ...
    if MULTI_TENANT:
        if team_name != get_current_tenant_id():
            user_count = fetch_ee_implementation_or_noop(
                "virchow.server.tenants.user_mapping", "get_tenant_count", None
            )(team_name)
        ...
        tenant_invitation = fetch_ee_implementation_or_noop(
            "virchow.server.tenants.user_mapping", "get_tenant_invitation", None
        )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "virchow.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
