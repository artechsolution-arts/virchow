...
from virchow.db.enums import SharingScope
...
from virchow.server.features.build.api.messages_api import router as messages_router
from virchow.server.features.build.api.models import BuildConnectorInfo
from virchow.server.features.build.api.models import BuildConnectorListResponse
from virchow.server.features.build.api.models import BuildConnectorStatus
from virchow.server.features.build.api.models import RateLimitResponse
from virchow.server.features.build.api.rate_limit import get_user_rate_limit_status
from virchow.server.features.build.api.sessions_api import router as sessions_router
from virchow.server.features.build.api.user_library import router as user_library_router
...
def require_virchow_craft_enabled(user: User = Depends(current_user)) -> User:
    """
    Dependency that checks if Virchow Craft is enabled for the user.
    Raises HTTP 403 if Virchow Craft is disabled via feature flag.
    """
    if not is_virchow_craft_enabled(user):
        raise HTTPException(
            status_code=403,
            detail="Virchow Craft is not available",
        )
    return user


router = APIRouter(prefix="/build", dependencies=[Depends(require_virchow_craft_enabled)])
...
# prevent LLM-generated apps from setting cookies on the parent Virchow domain.
EXCLUDED_HEADERS = {
    "content-encoding",
    "content-length",
    "transfer-encoding",
