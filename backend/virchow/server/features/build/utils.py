...
from virchow.db.notification import create_notification
from virchow.feature_flags.factory import get_default_feature_flag_provider
from virchow.feature_flags.interface import NoOpFeatureFlagProvider
...
from virchow.server.features.build.configs import ENABLE_CRAFT
from virchow.server.features.build.configs import MAX_UPLOAD_FILE_SIZE_BYTES
...
# Combine base Virchow extensions with code file extensions
ALLOWED_EXTENSIONS: set[str] = (
    VirchowFileExtensions.ALL_ALLOWED_EXTENSIONS | CODE_FILE_EXTENSIONS
)

# Combine base Virchow MIME types with code MIME types
ALLOWED_MIME_TYPES: set[str] = VirchowMimeTypes.ALLOWED_MIME_TYPES | CODE_MIME_TYPES
...
# PostHog feature flag key for enabling Virchow Craft (cloud rollout control)
# Flag logic: True = enabled, False/null/not found = disabled
VIRCHOW_CRAFT_ENABLED_FLAG = "virchow-craft-enabled"
...
def is_virchow_craft_enabled(user: User) -> bool:
    """
    Check if Virchow Craft (Build Mode) is enabled for the user.

    Flag logic for "virchow-craft-enabled":
    - Flag = True → enabled (Virchow Craft is available)
    - Flag = False → disabled (Virchow Craft is not available)
    - Flag = null/not found → disabled (Virchow Craft is not available)
    ...
    is_enabled = feature_flag_provider.feature_enabled(
        VIRCHOW_CRAFT_ENABLED_FLAG,
        user.id,
    )

    if is_enabled:
        logger.debug("Virchow Craft enabled via PostHog feature flag")
        return True
    else:
        logger.debug("Virchow Craft disabled via PostHog feature flag")
        return False
...
def ensure_build_mode_intro_notification(user: User, db_session: Session) -> None:
    ...
    """
    # PostHog feature flag check - only show notification if Virchow Craft is enabled
    ...
    create_notification(
        ...
        title="Introducing Virchow Craft",
        description="Unleash Virchow to create dashboards, slides, documents, and more with your connected data.",
