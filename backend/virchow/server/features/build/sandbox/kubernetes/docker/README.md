...
This directory contains the Dockerfile and resources for building the Virchow Craft sandbox container image.
...
```bash
cd backend/virchow/server/features/build/sandbox/kubernetes/docker
docker build --platform linux/amd64 -t virchowdotapp/sandbox:v0.1.x .
docker push virchowdotapp/sandbox:v0.1.x
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t virchowdotapp/sandbox:v0.1.x \
  --push .
```
...
```bash
docker tag virchowdotapp/sandbox:v0.1.x virchowdotapp/sandbox:latest
docker push virchowdotapp/sandbox:latest
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t virchowdotapp/sandbox:v0.1.x \
  -t virchowdotapp/sandbox:latest \
  --push .
```
...
2. **Update the ConfigMap** in `cloud-deployment-yamls/danswer/configmap/env-configmap.yaml`:
   ```yaml
   SANDBOX_CONTAINER_IMAGE: "virchowdotapp/sandbox:v0.1.x"
   ```
...
5. **Delete existing sandbox pods** (they will be recreated with the new image):
   ```bash
   kubectl delete pods -n virchow-sandboxes -l app.kubernetes.io/component=sandbox
   ```
...
```bash
curl -s "https://hub.docker.com/v2/repositories/virchowdotapp/sandbox/tags" | jq '.results[].name'
```

### Check what image a pod is using

```bash
kubectl get pod <pod-name> -n virchow-sandboxes -o jsonpath='{.spec.containers[?(@.name=="sandbox")].image}'
```
