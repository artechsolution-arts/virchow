...
#
# This script:
# 1. Builds the virchow-backend Docker image
...
IMAGE_NAME="virchowdotapp/virchow-backend:latest"
TEST_FILE="virchow/server/features/build/sandbox/kubernetes/test_kubernetes_sandbox.py"
...
echo "=== Building virchow-backend Docker image ==="
...
kind load docker-image "$IMAGE_NAME" --name virchow 2>/dev/null || \
