# Virchow Sandbox System

This directory contains the implementation of Virchow's sandbox system for running OpenCode agents in isolated environments.
...
cd backend
docker build -f Dockerfile.sandbox-templates -t virchowdotapp/backend:latest .

# Build sandbox container (lightweight runner)
cd virchow/server/features/build/sandbox/kubernetes/docker
docker build -t virchowdotapp/sandbox:latest .
...
**Only needed if you're running the Virchow backend outside of Docker.** Most developers use Docker and can skip this section.

If you're running the backend Python process directly on your machine, you need templates at `/templates/`:

#### Web Template

The web template is a lightweight Next.js app (Next.js 16, React 19, shadcn/ui, Recharts) checked into the codebase at `backend/virchow/server/features/build/templates/outputs/web/`.

For local development, create a symlink to this template:

```bash
sudo mkdir -p /templates/outputs
sudo ln -s $(pwd)/backend/virchow/server/features/build/templates/outputs/web /templates/outputs/web
```
...
cd backend
python -m virchow.server.features.build.sandbox.util.build_venv_template

# Or manually
python3 -m venv /templates/venv
/templates/venv/bin/pip install -r backend/virchow/server/features/build/sandbox/kubernetes/docker/initial-requirements.txt
```
...
SANDBOX_BASE_PATH=/tmp/virchow-sandboxes     # Default: /tmp/virchow-sandboxes
...
SANDBOX_NAMESPACE=virchow-sandboxes          # Default: virchow-sandboxes

# Container image
SANDBOX_CONTAINER_IMAGE=virchowdotapp/sandbox:latest

# S3 bucket for snapshots and files
SANDBOX_S3_BUCKET=virchow-sandbox-files      # Default: virchow-sandbox-files
...
- Check pod logs: `kubectl logs -n virchow-sandboxes sandbox-{sandbox-id}`
- Check init container: `kubectl logs -n virchow-sandboxes sandbox-{sandbox-id} -c file-sync`
- Verify init container completed: `kubectl describe pod -n virchow-sandboxes sandbox-{sandbox-id}`
...
- **Docker/K8s**: Check container logs: `kubectl logs -n virchow-sandboxes sandbox-{sandbox-id}`
- Verify npm install succeeded (check entrypoint.sh logs)
- Check that web template was copied: `kubectl exec -n virchow-sandboxes sandbox-{sandbox-id} -- ls /workspace/outputs/web`
...
```bash
export OUTPUTS_TEMPLATE_PATH=$HOME/.virchow/templates/outputs
export VENV_TEMPLATE_PATH=$HOME/.virchow/templates/venv

# Then symlink to your home directory
mkdir -p $HOME/.virchow/templates/outputs
ln -s $(pwd)/backend/virchow/server/features/build/templates/outputs/web $HOME/.virchow/templates/outputs/web
```
...
The lightweight Next.js template (`backend/virchow/server/features/build/templates/outputs/web/`) includes:
...
This template provides a modern development environment without the complexity of the full Virchow application, allowing agents to build custom UIs quickly.
