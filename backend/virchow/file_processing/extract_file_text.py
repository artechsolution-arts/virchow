...
from virchow.file_processing.file_types import PRESENTATION_MIME_TYPE
...
from virchow.file_processing.unstructured import get_unstructured_api_key
from virchow.file_processing.unstructured import unstructured_to_text
from virchow.utils.logger import setup_logger

if TYPE_CHECKING:
    from markitdown import MarkItDown
from virchow.configs.app_configs import USE_DOCLING_OCR
from virchow.file_processing.docling_ocr import DoclingOCR
logger = setup_logger()
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == VIRCHOW_METADATA_FILENAME:
                continue
...
def _extract_virchow_metadata(line: str) -> dict | None:
    """
    Example: first line has:
        <!-- VIRCHOW_METADATA={"title": "..."} -->
      or
        #VIRCHOW_METADATA={"title":"..."}
    """
    html_comment_pattern = r"<!--\s*VIRCHOW_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#VIRCHOW_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_virchow_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts Virchow metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_virchow_metadata:
            potential_meta = _extract_virchow_metadata(line)
...
def read_docx_file(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_virchow_metadata=False
        )
...
def extract_file_text(
    ...
    try:
        ...
        if extension in VirchowFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
def _extract_text_and_images(
    ...
    if content_type in VirchowMimeTypes.TEXT_MIME_TYPES:
    ...
    try:
        ...
        # PDF example: use Docling if enabled for high quality, otherwise fallback to standard PDF parsing
        if extension == ".pdf":
            if USE_DOCLING_OCR:
                try:
                    logger.info("Using Docling for PDF extraction")
                    converter = DoclingOCR()
                    text_content = converter.extract_text(file)
                    if text_content:
                        return ExtractionResult(
                            text_content=text_content, embedded_images=[], metadata={}
                        )
                except Exception as e:
                    logger.error(f"Docling PDF extraction failed: {e}. Falling back.")
                    file.seek(0)
        ...
        if extension in VirchowFileExtensions.PLAIN_TEXT_EXTENSIONS:
            return extract_result_from_text_file(file)
