...
from virchow.access.access import source_should_fetch_permissions_during_indexing
from virchow.background.indexing.checkpointing_utils import check_checkpoint_size
from virchow.background.indexing.checkpointing_utils import get_latest_valid_checkpoint
from virchow.background.indexing.checkpointing_utils import save_checkpoint
from virchow.background.indexing.memory_tracer import MemoryTracer
from virchow.configs.app_configs import INDEX_BATCH_SIZE
from virchow.configs.app_configs import INDEXING_SIZE_WARNING_THRESHOLD
from virchow.configs.app_configs import INDEXING_TRACER_INTERVAL
from virchow.configs.app_configs import INTEGRATION_TESTS_MODE
from virchow.configs.app_configs import LEAVE_CONNECTOR_ACTIVE_ON_INITIALIZATION_FAILURE
from virchow.configs.app_configs import MAX_FILE_SIZE_BYTES
from virchow.configs.app_configs import POLL_CONNECTOR_OFFSET
...
from virchow.connectors.connector_runner import ConnectorRunner
from virchow.connectors.exceptions import ConnectorValidationError
from virchow.connectors.exceptions import UnexpectedValidationError
...
from virchow.connectors.models import ConnectorStopSignal
from virchow.connectors.models import Document
from virchow.connectors.models import IndexAttemptMetadata
from virchow.connectors.models import TextSection
...
from virchow.db.connector_credential_pair import get_last_successful_attempt_poll_range_end
from virchow.db.connector_credential_pair import update_connector_credential_pair
from virchow.db.constants import CONNECTOR_VALIDATION_ERROR_MESSAGE_PREFIX
...
from virchow.db.enums import IndexModelStatus
...
from virchow.db.index_attempt import get_recent_completed_attempts_for_cc_pair
from virchow.db.index_attempt import mark_attempt_canceled
from virchow.db.index_attempt import mark_attempt_failed
from virchow.db.index_attempt import transition_attempt_to_in_progress
...
from virchow.server.features.build.indexing.persistent_document_writer import (
...
from virchow.utils.postgres_sanitization import sanitize_document_for_postgres
from virchow.utils.postgres_sanitization import sanitize_hierarchy_nodes_for_postgres
from virchow.utils.variable_functionality import global_version
...
def connector_document_extraction(
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        docprocessing_priority = (
            VirchowCeleryPriority.MEDIUM
            if has_successful_attempt
            else VirchowCeleryPriority.HIGH
        )
        ...
        # new channels could be missed (since existing channels are
    ...
    try:
                ...
                if processing_mode == ProcessingMode.FILE_SYSTEM:
                    ...
                    with get_session_with_current_tenant() as db_session:
                        ...
                        index_attempt_metadata = IndexAttemptMetadata(
                            ...
                            connector_id=db_connector.id,
                            credential_id=db_credential.id,
                            request_id=make_randomized_virchow_request_id("FSI"),
                    ...
                    app.send_task(
                        VirchowCeleryTask.DOCPROCESSING_TASK,
                        kwargs=processing_batch_data,
                        queue=VirchowCeleryQueues.DOCPROCESSING,
        ...
        if processing_mode == ProcessingMode.FILE_SYSTEM:
            ...
            if creator_id:
                ...
                app.send_task(
                    VirchowCeleryTask.SANDBOX_FILE_SYNC,
                    kwargs={
...
def reissue_old_batches(
    ...
    priority: VirchowCeleryPriority,
) -> tuple[int, int]:
