...
from virchow.background.celery.celery_redis import celery_find_task
from virchow.background.celery.celery_redis import celery_get_unacked_task_ids
from virchow.background.celery.celery_utils import httpx_init_vespa_pool
from virchow.background.celery.memory_monitoring import emit_process_memory
from virchow.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from virchow.background.celery.tasks.docfetching.task_creation_utils import (
    try_creating_docfetching_task,
)
from virchow.background.celery.tasks.docprocessing.heartbeat import start_heartbeat
from virchow.background.celery.tasks.docprocessing.heartbeat import stop_heartbeat
from virchow.background.celery.tasks.docprocessing.utils import IndexingCallback
from virchow.background.celery.tasks.docprocessing.utils import is_in_repeated_error_state
from virchow.background.celery.tasks.docprocessing.utils import should_index
from virchow.background.celery.tasks.models import DocProcessingContext
from virchow.background.indexing.checkpointing_utils import cleanup_checkpoint
from virchow.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from virchow.background.indexing.index_attempt_utils import cleanup_index_attempts
from virchow.background.indexing.index_attempt_utils import get_old_index_attempts
from virchow.configs.app_configs import AUTH_TYPE
from virchow.configs.app_configs import MANAGED_VESPA
from virchow.configs.app_configs import VESPA_CLOUD_CERT_PATH
from virchow.configs.app_configs import VESPA_CLOUD_KEY_PATH
from virchow.configs.constants import AuthType
from virchow.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from virchow.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from virchow.configs.constants import VirchowRedisSignals
from virchow.connectors.models import ConnectorFailure
...
from virchow.db.connector_credential_pair import set_cc_pair_repeated_error_state
from virchow.db.connector_credential_pair import update_connector_credential_pair_from_id
from virchow.db.engine.sql_engine import get_session_with_current_tenant
from virchow.db.engine.time_utils import get_db_current_time
from virchow.db.enums import ConnectorCredentialPairStatus
from virchow.db.enums import IndexingMode
from virchow.db.enums import IndexingStatus
from virchow.db.enums import SwitchoverType
from virchow.db.index_attempt import create_index_attempt_error
from virchow.db.index_attempt import get_index_attempt
from virchow.db.index_attempt import get_index_attempt_errors_for_cc_pair
from virchow.db.index_attempt import IndexAttemptError
from virchow.db.index_attempt import mark_attempt_canceled
from virchow.db.index_attempt import mark_attempt_failed
from virchow.db.index_attempt import mark_attempt_partially_succeeded
from virchow.db.index_attempt import mark_attempt_succeeded
from virchow.db.indexing_coordination import CoordinationStatus
from virchow.db.indexing_coordination import INDEXING_PROGRESS_TIMEOUT_HOURS
from virchow.db.indexing_coordination import IndexingCoordination
from virchow.db.models import IndexAttempt
from virchow.db.models import SearchSettings
...
from virchow.file_store.document_batch_storage import DocumentBatchStorage
from virchow.file_store.document_batch_storage import get_document_batch_storage
from virchow.httpx.httpx_pool import HttpxPool
...
from virchow.redis.redis_connector import RedisConnector
from virchow.redis.redis_pool import get_redis_client
from virchow.redis.redis_pool import get_redis_replica_client
from virchow.redis.redis_pool import redis_lock_dump
from virchow.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from virchow.redis.redis_utils import is_fence
from virchow.server.runtime.virchow_runtime import VirchowRuntime
from virchow.utils.logger import setup_logger
from virchow.utils.middleware import make_randomized_virchow_request_id
from virchow.utils.telemetry import mt_cloud_telemetry
from virchow.utils.telemetry import optional_telemetry
from virchow.utils.telemetry import RecordType
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = VirchowRuntime.get_beat_multiplier()
    except Exception:
        ...
        if attempt and timed_out:
            if attempt.status == IndexingStatus.IN_PROGRESS:
                ...
                task_exists = celery_find_task(
                    ...
                    VirchowCeleryQueues.CONNECTOR_DOC_FETCHING,
                    redis_celery,
                )
                unacked_task_ids = celery_get_unacked_task_ids(
                    VirchowCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
                )
...
@shared_task(
    name=VirchowCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (VirchowCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        VirchowRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(VirchowRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    VirchowRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(VirchowRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                VirchowRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=VirchowRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(VirchowRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                VirchowRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=VirchowCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        VirchowRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    VirchowCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=VirchowCeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=VirchowCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=VirchowCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        VirchowRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempts), batch_size):
                ...
                self.app.send_task(
                    VirchowCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": [attempt.id for attempt in batch],
                        "tenant_id": tenant_id,
                    },
                    queue=VirchowCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=VirchowCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=VirchowCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=VirchowCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
...
def _check_chunk_usage_limit(tenant_id: str) -> None:
    ...
    from virchow.db.usage import UsageType
    from virchow.server.usage_limits import check_usage_and_raise
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_virchow_request_id("DIP"),
        ...
        if USAGE_LIMITS_ENABLED and index_pipeline_result.total_chunks > 0:
            try:
                from virchow.db.usage import increment_usage
                from virchow.db.usage import UsageType
