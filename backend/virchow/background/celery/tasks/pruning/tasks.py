...
from virchow.background.celery.celery_redis import celery_get_queued_task_ids
from virchow.background.celery.celery_redis import celery_get_unacked_task_ids
from virchow.background.celery.celery_utils import extract_ids_from_runnable_connector
from virchow.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from virchow.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from virchow.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
from virchow.configs.app_configs import JOB_TIMEOUT
from virchow.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from virchow.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
from virchow.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from virchow.configs.constants import DANSWER_REDIS_FUNCTION_LOCK_PREFIX
...
from virchow.connectors.factory import instantiate_connector
from virchow.connectors.models import InputType
from virchow.db.connector import mark_ccpair_as_pruned
...
from virchow.db.document import get_documents_for_connector_credential_pair
...
from virchow.db.hierarchy import delete_orphaned_hierarchy_nodes
from virchow.db.hierarchy import link_hierarchy_nodes_to_documents
from virchow.db.hierarchy import remove_stale_hierarchy_node_cc_pair_entries
from virchow.db.hierarchy import reparent_orphaned_hierarchy_nodes
from virchow.db.hierarchy import update_document_parent_hierarchy_nodes
from virchow.db.hierarchy import upsert_hierarchy_node_cc_pair_entries
from virchow.db.hierarchy import upsert_hierarchy_nodes_batch
from virchow.db.models import ConnectorCredentialPair
from virchow.db.models import HierarchyNode as DBHierarchyNode
from virchow.db.sync_record import insert_sync_record
from virchow.db.sync_record import update_sync_record_status
from virchow.db.tag import delete_orphan_tags__no_commit
...
from virchow.redis.redis_connector_prune import RedisConnectorPrunePayload
from virchow.redis.redis_hierarchy import cache_hierarchy_nodes_batch
from virchow.redis.redis_hierarchy import ensure_source_node_exists
from virchow.redis.redis_hierarchy import evict_hierarchy_nodes_from_cache
from virchow.redis.redis_hierarchy import get_node_id_from_raw_id
from virchow.redis.redis_hierarchy import get_source_node_id_from_cache
from virchow.redis.redis_hierarchy import HierarchyNodeCacheEntry
...
from virchow.server.utils import make_short_id
from virchow.utils.logger import format_error_for_logging
from virchow.utils.logger import LoggerContextVars
from virchow.utils.logger import pruning_ctx
...
@shared_task(
    name=VirchowCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        VirchowRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(VirchowRedisSignals.BLOCK_PRUNING):
            ...
            r.set(VirchowRedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(VirchowRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                VirchowRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
        ...
        lock_beat.reacquire()
        keys = cast(set[Any], r_replica.smembers(VirchowRedisConstants.ACTIVE_FENCES))
        for key in keys:
            key_bytes = cast(bytes, key)

            if not r.exists(key_bytes):
                r.srem(VirchowRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            VirchowCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=VirchowCeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(
    name=VirchowCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        VirchowRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(VirchowCeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        VirchowCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
