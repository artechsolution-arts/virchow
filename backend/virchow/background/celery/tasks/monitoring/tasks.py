...
from virchow.background.celery.celery_redis import celery_get_queue_length
...
from virchow.configs.constants import VIRCHOW_CLOUD_TENANT_ID
...
from virchow.db.engine.sql_engine import get_session_with_shared_schema
from virchow.db.engine.tenant_utils import get_all_tenant_ids
from virchow.db.engine.time_utils import get_db_current_time
from virchow.db.enums import IndexingStatus
from virchow.db.enums import SyncStatus
from virchow.db.enums import SyncType
from virchow.db.models import ConnectorCredentialPair
from virchow.db.models import DocumentSet
from virchow.db.models import IndexAttempt
from virchow.db.models import SyncRecord
from virchow.db.models import UserGroup
from virchow.db.search_settings import get_active_search_settings_list
from virchow.redis.redis_pool import get_redis_client
from virchow.redis.redis_pool import redis_lock_dump
from virchow.utils.logger import is_running_in_container
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    metrics = []
    queue_mappings = {
        "celery_queue_length": VirchowCeleryQueues.PRIMARY,
        "docprocessing_queue_length": VirchowCeleryQueues.DOCPROCESSING,
        "docfetching_queue_length": VirchowCeleryQueues.CONNECTOR_DOC_FETCHING,
        "sync_queue_length": VirchowCeleryQueues.VESPA_METADATA_SYNC,
        "deletion_queue_length": VirchowCeleryQueues.CONNECTOR_DELETION,
        "pruning_queue_length": VirchowCeleryQueues.CONNECTOR_PRUNING,
        "permissions_sync_queue_length": VirchowCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": VirchowCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": VirchowCeleryQueues.DOC_PERMISSIONS_UPSERT,
        "hierarchy_fetching_queue_length": VirchowCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
        "llm_model_update_queue_length": VirchowCeleryQueues.LLM_MODEL_UPDATE,
        "checkpoint_cleanup_queue_length": VirchowCeleryQueues.CHECKPOINT_CLEANUP,
        "index_attempt_cleanup_queue_length": VirchowCeleryQueues.INDEX_ATTEMPT_CLEANUP,
        "csv_generation_queue_length": VirchowCeleryQueues.CSV_GENERATION,
        "user_file_processing_queue_length": VirchowCeleryQueues.USER_FILE_PROCESSING,
        "user_file_project_sync_queue_length": VirchowCeleryQueues.USER_FILE_PROJECT_SYNC,
        "user_file_delete_queue_length": VirchowCeleryQueues.USER_FILE_DELETE,
        "monitoring_queue_length": VirchowCeleryQueues.MONITORING,
        "sandbox_queue_length": VirchowCeleryQueues.SANDBOX,
        "opensearch_migration_queue_length": VirchowCeleryQueues.OPENSEARCH_MIGRATION,
    }
...
@shared_task(
    name=VirchowCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
...
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        VirchowRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=VirchowCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    redis_client = get_redis_client(tenant_id=VIRCHOW_CLOUD_TENANT_ID)

    lock_beat: RedisLock = redis_client.lock(
        VirchowRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=VirchowCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=VirchowCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    n_celery = celery_get_queue_length(VirchowCeleryQueues.PRIMARY, r_celery)
    n_docfetching = celery_get_queue_length(
        VirchowCeleryQueues.CONNECTOR_DOC_FETCHING, r_celery
    )
    n_docprocessing = celery_get_queue_length(VirchowCeleryQueues.DOCPROCESSING, r_celery)

    n_user_file_processing = celery_get_queue_length(
        VirchowCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        VirchowCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    n_user_file_delete = celery_get_queue_length(
        VirchowCeleryQueues.USER_FILE_DELETE, r_celery
    )
    n_sync = celery_get_queue_length(VirchowCeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(VirchowCeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(VirchowCeleryQueues.CONNECTOR_PRUNING, r_celery)
    n_permissions_sync = celery_get_queue_length(
        VirchowCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
    n_external_group_sync = celery_get_queue_length(
        VirchowCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    n_permissions_upsert = celery_get_queue_length(
        VirchowCeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    n_hierarchy_fetching = celery_get_queue_length(
        VirchowCeleryQueues.CONNECTOR_HIERARCHY_FETCHING, r_celery
    )
    n_llm_model_update = celery_get_queue_length(
        VirchowCeleryQueues.LLM_MODEL_UPDATE, r_celery
    )
    n_checkpoint_cleanup = celery_get_queue_length(
        VirchowCeleryQueues.CHECKPOINT_CLEANUP, r_celery
    )
    n_index_attempt_cleanup = celery_get_queue_length(
        VirchowCeleryQueues.INDEX_ATTEMPT_CLEANUP, r_celery
    )
    n_csv_generation = celery_get_queue_length(
        VirchowCeleryQueues.CSV_GENERATION, r_celery
    )
    n_monitoring = celery_get_queue_length(VirchowCeleryQueues.MONITORING, r_celery)
    n_sandbox = celery_get_queue_length(VirchowCeleryQueues.SANDBOX, r_celery)
    n_opensearch_migration = celery_get_queue_length(
        VirchowCeleryQueues.OPENSEARCH_MIGRATION, r_celery
    )
    ...
    n_docprocessing_prefetched = celery_get_unacked_task_ids(
        VirchowCeleryQueues.DOCPROCESSING, r_celery
    )
...
@shared_task(
    name=VirchowCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=VirchowCeleryQueues.MONITORING,
...
@shared_task(
    name=VirchowCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
