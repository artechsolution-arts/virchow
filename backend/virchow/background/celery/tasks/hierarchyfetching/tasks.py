...
from virchow.connectors.factory import ConnectorMissingException
from virchow.connectors.factory import identify_connector_class
from virchow.connectors.factory import instantiate_connector
from virchow.connectors.interfaces import HierarchyConnector
from virchow.connectors.models import HierarchyNode as PydanticHierarchyNode
from virchow.db.connector import mark_cc_pair_as_hierarchy_fetched
...
def _try_creating_hierarchy_fetching_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            VirchowCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair.id,
                tenant_id=tenant_id,
            ),
            queue=VirchowCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
...
@shared_task(
    name=VirchowCeleryTask.CHECK_FOR_HIERARCHY_FETCHING,
    soft_time_limit=300,
    bind=True,
)
def check_for_hierarchy_fetching(self: Task, *, tenant_id: str) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        VirchowRedisLocks.CHECK_HIERARCHY_FETCHING_BEAT_LOCK,
...
@shared_task(
    name=VirchowCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
