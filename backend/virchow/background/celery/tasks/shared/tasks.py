...
from virchow.configs.constants import VIRCHOW_CELERY_BEAT_HEARTBEAT_KEY
from virchow.configs.constants import VirchowCeleryTask
from virchow.db.document import delete_document_by_connector_credential_pair__no_commit
from virchow.db.document import delete_documents_complete__no_commit
from virchow.db.document import fetch_chunk_count_for_document
from virchow.db.document import get_document
from virchow.db.document import get_document_connector_count
from virchow.db.document import mark_document_as_modified
...
from virchow.db.relationships import delete_document_references_from_kg
...
class VirchowCeleryTaskCompletionStatus(str, Enum):
...
@shared_task(
    name=VirchowCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
...
def document_by_cc_pair_cleanup_task(
    ...
    if completion_status != VirchowCeleryTaskCompletionStatus.SUCCEEDED:
        return False
...
@shared_task(name=VirchowCeleryTask.CELERY_BEAT_HEARTBEAT, ignore_result=True, bind=True)
def celery_beat_heartbeat(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    r.set(VIRCHOW_CELERY_BEAT_HEARTBEAT_KEY, 1, ex=600)
