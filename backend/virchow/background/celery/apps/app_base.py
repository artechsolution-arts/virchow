...
from virchow.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from virchow.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from virchow.background.celery.celery_utils import celery_is_worker_primary
from virchow.background.celery.celery_utils import make_probe_path
from virchow.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_PREFIX
from virchow.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_TASKSET_KEY
...
from virchow.db.engine.sql_engine import get_sqlalchemy_engine
from virchow.document_index.opensearch.client import (
...
from virchow.tracing.setup import setup_tracing
from virchow.utils.logger import ColoredFormatter
from virchow.utils.logger import LoggerContextVars
from virchow.utils.logger import PlainFormatter
...
def on_task_postrun(
    ...
    **kwds: Any,  # noqa: ARG001
    ...
    if task.name.startswith(VIRCHOW_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    while True:
        if r.exists(VirchowRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in VirchowLoggingAdapter
...
_VECTOR_DB_TASK_MODULES: set[str] = {
    ...
    "virchow.background.celery.tasks.docfetching",
    "virchow.background.celery.tasks.pruning",
    "virchow.background.celery.tasks.vespa",
    "virchow.background.celery.tasks.opensearch_migration",
    "virchow.background.celery.tasks.doc_permission_syncing",
    "virchow.background.celery.tasks.hierarchyfetching",
    # EE modules that are vector-DB-dependent
    "ee.virchow.background.celery.tasks.doc_permission_syncing",
    "ee.virchow.background.celery.tasks.external_group_syncing",
}
# NOTE: "virchow.background.celery.tasks.shared" is intentionally NOT in the set
