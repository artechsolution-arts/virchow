...
def _run_cache_cleanup() -> None:
    from virchow.cache.postgres_backend import cleanup_expired_cache_entries
...
def _build_periodic_tasks() -> list[_PeriodicTaskDef]:
    """No periodic tasks in minimal RAG."""
    return []
...
def _run_drain_loops(tenant_id: str) -> None:
    from virchow.background.task_utils import drain_rag_upload_loop

    drain_rag_upload_loop()
