...
from virchow.configs.app_configs import INDEXING_EMBEDDING_MODEL_NUM_THREADS
from virchow.configs.app_configs import LARGE_CHUNK_RATIO
from virchow.configs.model_configs import BATCH_SIZE_ENCODE_CHUNKS
from virchow.configs.model_configs import (
...
from virchow.natural_language_processing.constants import DEFAULT_COHERE_MODEL
from virchow.natural_language_processing.constants import DEFAULT_OPENAI_MODEL
from virchow.natural_language_processing.constants import DEFAULT_VERTEX_MODEL
from virchow.natural_language_processing.constants import DEFAULT_VOYAGE_MODEL
from virchow.natural_language_processing.constants import EmbeddingModelTextType
from virchow.natural_language_processing.exceptions import CohereBillingLimitError
from virchow.natural_language_processing.exceptions import ModelServerRateLimitError
from virchow.natural_language_processing.utils import get_tokenizer
from virchow.natural_language_processing.utils import tokenizer_trim_content
from virchow.utils.logger import setup_logger
from virchow.utils.search_nlp_models_utils import pass_aws_key
...
WARM_UP_STRINGS = [
    ...
    "https://docs.virchow.app/deployment/getting_started/quickstart",
]
...
class CloudEmbedding:
    ...
    async def _embed_ollama(
        ...
        if not self.api_url:
            raise ValueError("API URL is required for Ollama embedding.")

        # Ensure the URL points to /api/embed if not already
        url = self.api_url
        if not url.endswith("/api/embed") and not url.endswith("/api/embed/"):
            url = url.rstrip("/") + "/api/embed"
        ...
        return cast(list[Embedding], result["embeddings"])
    ...
    async def embed(
        ...
        *,
        ...
        deployment_name: str | None = None,  # noqa: ARG002
        reduced_dimension: int | None = None,  # noqa: ARG002
    ) -> list[Embedding]:
        try:
            if self.provider == EmbeddingProvider.OLLAMA:
                return await self._embed_ollama(texts, model_name)
            else:
                raise ValueError(f"Unsupported provider: {self.provider}. Only Ollama is supported in this on-premises deployment.")
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Virchow-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Virchow-Request-ID"] = request_id
