...
from virchow.configs.app_configs import AVERAGE_SUMMARY_EMBEDDINGS
from virchow.configs.app_configs import BLURB_SIZE
from virchow.configs.app_configs import LARGE_CHUNK_RATIO
from virchow.configs.app_configs import MINI_CHUNK_SIZE
from virchow.configs.app_configs import SKIP_METADATA_IN_CHUNK
...
from virchow.configs.constants import SECTION_SEPARATOR
...
from virchow.configs.app_configs import USE_INVOICE_CHUNKER
from virchow.indexing.chunking.invoice_chunker import InvoiceChunker
from virchow.utils.logger import setup_logger
from virchow.utils.text_processing import clean_text
from virchow.utils.text_processing import shared_precompare_cleanup
...
class Chunker:
    ...
    def _handle_single_document(
        ...
        if USE_INVOICE_CHUNKER:
            logger.info(f"Using InvoiceChunker for document: {document.id}")
            invoice_chunker = InvoiceChunker()
            invoice_chunks_raw = invoice_chunker.chunk_invoice_data(document.content)
            
            chunks: list[DocAwareChunk] = []
            for chunk_raw in invoice_chunks_raw:
                self._create_chunk(
                    document=document,
                    chunks_list=chunks,
                    text=chunk_raw["content"],
                    links={0: ""},
                    is_continuation=False,
                )
            return chunks
