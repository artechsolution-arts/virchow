...
from virchow.utils.text_processing import make_url_compatible
...
class DocumentBase(BaseModel):
    """Used for Virchow ingestion api, the ID is inferred before use if not provided"""
    ...
    # different because some identifiers may display as structural (e.g. #general) but
    # should not be part of the search, at least not in the same way as a document title.
...
class Document(DocumentBase):
    """Used for Virchow ingestion api, the ID is required"""
...
class VirchowMetadata(BaseModel):
