...
from virchow.configs.app_configs import REQUEST_TIMEOUT_SECONDS
from virchow.configs.app_configs import SHAREPOINT_CONNECTOR_SIZE_THRESHOLD
...
from virchow.connectors.interfaces import IndexingHeartbeatInterface
...
from virchow.connectors.microsoft_graph_env import resolve_microsoft_environment
from virchow.connectors.models import BasicExpertInfo
from virchow.connectors.models import ConnectorCheckpoint
...
from virchow.connectors.models import ExternalAccess
...
from virchow.connectors.sharepoint.connector_utils import get_sharepoint_external_access
from virchow.db.enums import HierarchyNodeType
from virchow.file_processing.extract_file_text import extract_text_and_images
from virchow.file_processing.extract_file_text import get_file_ext
from virchow.file_processing.file_types import VirchowFileExtensions
from virchow.file_processing.file_types import VirchowMimeTypes
from virchow.file_processing.image_utils import store_image_and_create_section
...
def _convert_driveitem_to_document_with_permissions(
    ...
    if not mime_type or mime_type in VirchowMimeTypes.EXCLUDED_IMAGE_TYPES:
        logger.debug(
    ...
    if not content_bytes:
    ...
    elif file_ext in VirchowFileExtensions.IMAGE_EXTENSIONS:
        ...
        def _store_embedded_image(img_data: bytes, img_name: str) -> None:
            ...
            if img_mime in VirchowMimeTypes.EXCLUDED_IMAGE_TYPES:
                logger.debug(
    ...
    def _load_from_checkpoint(
        ...
        if (
            ...
            for driveitem in driveitems:
                ...
                if driveitem_extension not in VirchowFileExtensions.ALL_ALLOWED_EXTENSIONS:
                    logger.warning(
                ...
                should_yield_if_empty = (
                    driveitem_extension in VirchowFileExtensions.IMAGE_EXTENSIONS
