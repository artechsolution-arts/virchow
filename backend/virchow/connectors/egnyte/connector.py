...
from virchow.configs.app_configs import EGNYTE_CLIENT_ID
from virchow.configs.app_configs import EGNYTE_CLIENT_SECRET
...
from virchow.connectors.interfaces import OAuthConnector
...
from virchow.file_processing.extract_file_text import detect_encoding
...
from virchow.file_processing.extract_file_text import read_text_file
from virchow.file_processing.file_types import VirchowFileExtensions
from virchow.utils.logger import setup_logger
from virchow.utils.retry_wrapper import request_with_retries
...
def _process_egnyte_file(
    ...
    if extension not in VirchowFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
        logger.warning(f"Skipping file '{file_name}' with extension '{extension}'")
        return None

    # Extract text content based on file type
    # TODO @wenxi-virchow: convert to extract_text_and_images
    if extension in VirchowFileExtensions.PLAIN_TEXT_EXTENSIONS:
        encoding = detect_encoding(file_content)
        file_content_raw, file_metadata = read_text_file(
            file_content, encoding=encoding, ignore_virchow_metadata=False
        )
    else:
