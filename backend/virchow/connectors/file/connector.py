...
def _process_file(
    ...
    if extension not in VirchowFileExtensions.ALL_ALLOWED_EXTENSIONS:
        logger.warning(
    ...
    virchow_metadata, custom_tags = process_virchow_metadata(metadata)
    file_display_name = virchow_metadata.file_display_name or os.path.basename(file_name)
    time_updated = virchow_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = virchow_metadata.primary_owners
    secondary_owners = virchow_metadata.secondary_owners
    link = virchow_metadata.link

    # These metadata items are not settable by the user
    source_type = virchow_metadata.source_type or DocumentSource.FILE

    doc_id = virchow_metadata.document_id or f"FILE_CONNECTOR__{file_id}"
    title = metadata.get("title") or file_display_name

    # 1) If the file itself is an image, handle that scenario quickly
    if extension in VirchowFileExtensions.IMAGE_EXTENSIONS:
        # Read the image data
    ...
    # Each file may have file-specific VIRCHOW_METADATA https://docs.virchow.app/admins/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        virchow_metadata, more_custom_tags = process_virchow_metadata(
        ...
        source_type = virchow_metadata.source_type or source_type
        primary_owners = virchow_metadata.primary_owners or primary_owners
        secondary_owners = virchow_metadata.secondary_owners or secondary_owners
        time_updated = virchow_metadata.doc_updated_at or time_updated
        file_display_name = virchow_metadata.file_display_name or file_display_name
        title = virchow_metadata.title or virchow_metadata.file_display_name or title
        link = virchow_metadata.link or link
