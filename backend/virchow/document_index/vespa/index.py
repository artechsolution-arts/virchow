...
from virchow.document_index.vespa.vespa_document_index import TenantState
...
from virchow.document_index.vespa_constants import VESPA_APPLICATION_ENDPOINT
...
from virchow.key_value_store.factory import get_shared_kv_store
from virchow.kg.utils.formatting_utils import split_relationship_id
...
class VespaIndex(DocumentIndex):
    ...
    def ensure_indices_exist(
        ...
        vespa_schema_path = os.path.join(
            os.getcwd(), "virchow", "document_index", "vespa", "app_config"
        )
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to prepare Vespa Virchow Index. Response: {response.text}"
            )
            raise RuntimeError(
                f"Failed to prepare Vespa Virchow Index. Response: {response.text}"
            )
    ...
    def register_multitenant_indices(
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to prepare Vespa Virchow Indexes. Response: {response.text}"
            )
