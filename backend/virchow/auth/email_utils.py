...
from virchow.configs.app_configs import EMAIL_FROM
from virchow.configs.app_configs import SENDGRID_API_KEY
from virchow.configs.app_configs import SMTP_PASS
from virchow.configs.app_configs import SMTP_PORT
from virchow.configs.app_configs import SMTP_SERVER
from virchow.configs.app_configs import SMTP_USER
...
def build_html_email(
    ...
    community_link_fragment = ""
    if application_name == VIRCHOW_DEFAULT_APPLICATION_NAME:
        community_link_fragment = f'<br>Have questions? Join our Discord community <a href="{VIRCHOW_DISCORD_URL}">here</a>.'
...
def send_email_with_sendgrid(
    ...
    from_email = Email(mail_from) if mail_from else Email("noreply@virchow.app")
...
def send_email_with_smtplib(
    ...
    msg["Message-ID"] = make_msgid(domain="virchow.app")
...
def send_subscription_cancellation_email(user_email: str) -> None:
    ...
    try:
        load_runtime_settings_fn = fetch_versioned_implementation(
            "virchow.server.enterprise_settings.store", "load_runtime_settings"
        )
    ...
    cta_link = "https://www.virchow.app/pricing"
    ...
    text_content = (
        ...
        "If you change your mind, visit https://www.virchow.app/pricing"
    )
    send_email(
        user_email,
        subject,
