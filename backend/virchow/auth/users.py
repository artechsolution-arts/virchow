...
from virchow.auth.api_key import get_hashed_api_key_from_request
from virchow.auth.disposable_email_validator import is_disposable_email
from virchow.auth.email_utils import send_forgot_password_email
from virchow.auth.email_utils import send_user_verification_email
...
from virchow.auth.jwt import verify_jwt_token
from virchow.auth.pat import get_hashed_pat_from_request
from virchow.auth.schemas import AuthBackend
from virchow.auth.schemas import UserCreate
from virchow.auth.schemas import UserRole
from virchow.auth.schemas import UserUpdateWithRole
from virchow.configs.app_configs import AUTH_BACKEND
from virchow.configs.app_configs import AUTH_COOKIE_EXPIRE_TIME_SECONDS
from virchow.configs.app_configs import AUTH_TYPE
from virchow.configs.app_configs import EMAIL_CONFIGURED
from virchow.configs.app_configs import JWT_PUBLIC_KEY_URL
from virchow.configs.app_configs import PASSWORD_MAX_LENGTH
from virchow.configs.app_configs import PASSWORD_MIN_LENGTH
from virchow.configs.app_configs import PASSWORD_REQUIRE_DIGIT
from virchow.configs.app_configs import PASSWORD_REQUIRE_LOWERCASE
from virchow.configs.app_configs import PASSWORD_REQUIRE_SPECIAL_CHAR
from virchow.configs.app_configs import PASSWORD_REQUIRE_UPPERCASE
from virchow.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from virchow.configs.app_configs import REQUIRE_EMAIL_VERIFICATION
from virchow.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from virchow.configs.app_configs import TRACK_EXTERNAL_IDP_EXPIRY
from virchow.configs.app_configs import USER_AUTH_SECRET
from virchow.configs.app_configs import VALID_EMAIL_DOMAINS
from virchow.configs.app_configs import WEB_DOMAIN
from virchow.configs.constants import ANONYMOUS_USER_COOKIE_NAME
from virchow.configs.constants import ANONYMOUS_USER_EMAIL
from virchow.configs.constants import ANONYMOUS_USER_UUID
from virchow.configs.constants import AuthType
from virchow.configs.constants import DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN
from virchow.configs.constants import DANSWER_API_KEY_PREFIX
...
from virchow.configs.constants import PASSWORD_SPECIAL_CHARS
from virchow.configs.constants import UNNAMED_KEY_PLACEHOLDER
from virchow.db.api_key import fetch_user_for_api_key
from virchow.db.auth import get_access_token_db
from virchow.db.auth import get_default_admin_user_emails
from virchow.db.auth import get_user_count
from virchow.db.auth import get_user_db
from virchow.db.auth import SQLAlchemyUserAdminDB
from virchow.db.engine.async_sql_engine import get_async_session
from virchow.db.engine.async_sql_engine import get_async_session_context_manager
from virchow.db.engine.sql_engine import get_session_with_current_tenant
from virchow.db.engine.sql_engine import get_session_with_tenant
from virchow.db.models import AccessToken
from virchow.db.models import OAuthAccount
from virchow.db.models import Persona
from virchow.db.models import User
from virchow.db.pat import fetch_user_for_pat
from virchow.db.users import get_user_by_email
from virchow.error_handling.error_codes import VirchowErrorCode
from virchow.error_handling.exceptions import log_virchow_error
from virchow.error_handling.exceptions import virchow_error_to_json_response
from virchow.error_handling.exceptions import VirchowError
from virchow.redis.redis_pool import get_async_redis_connection
from virchow.redis.redis_pool import retrieve_ws_token_data
...
from virchow.utils.telemetry import mt_cloud_identify
...
from virchow.utils.timing import log_function_time
from virchow.utils.url import add_url_params
...
def anonymous_user_enabled(*, tenant_id: str | None = None) -> bool:
    from virchow.cache.factory import get_cache_backend

    cache = get_cache_backend(tenant_id=tenant_id)
    value = cache.get(VirchowRedisLocks.ANONYMOUS_USER_ENABLED)
...
def verify_email_domain(email: str) -> None:
    ...
    if AUTH_TYPE == AuthType.CLOUD:
        ...
        if "+" in local_part and domain != "virchow.app":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
...
def enforce_seat_limit(db_session: Session, seats_needed: int = 1) -> None:
    ...
    result = fetch_ee_implementation_or_noop(
        "virchow.db.license", "check_seat_availability", None
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def create(
        ...
        from virchow.auth.captcha import CaptchaVerificationError
        from virchow.auth.captcha import is_captcha_enabled
        from virchow.auth.captcha import verify_captcha_token
        ...
        tenant_id = await fetch_ee_implementation_or_noop(
            "virchow.server.tenants.provisioning",
    ...
    async def on_after_register(
        ...
        get_marketing_posthog_cookie_name = fetch_ee_implementation_or_noop(
            module="virchow.utils.posthog_client",
        ...
        if (
            ...
            properties = {
                "email": user.email,
                "virchow_cloud_user_id": str(user.id),
...
class FastAPIUserWithLogoutRouter(FastAPIUsers[models.UP, models.ID]):
    ...
    def get_refresh_router(
        ...
        from virchow.auth.oauth_refresher import check_and_refresh_oauth_tokens
...
async def optional_user(
    ...
    if request.cookies.get("fastapiusersauth"):
        mock_user = User(
        ...
        # Mock additional fields that might be expected downstream
        mock_user.preferences = {"default_model": None}
        mock_user.pinned_assistants = []
        return mock_user
    return None
...
def get_default_admin_user_emails_() -> list[str]:
    # No default seeding available for Virchow MIT
    return []
...
def create_virchow_oauth_router(
...
def get_oauth_router(
    ...
    *,
    ...
    async def callback(
        ...
        def build_error_response(exc: VirchowError) -> JSONResponse:
            log_virchow_error(exc)
            error_response = virchow_error_to_json_response(exc)
        ...
        def decode_and_validate_state(state_value: str) -> Dict[str, str]:
            try:
                ...
                raise VirchowError(
                    VirchowErrorCode.VALIDATION_ERROR,
                    getattr(
                        ErrorCode,
        ...
        if enable_pkce:
            ...
            if access_token_state is None:
                raise VirchowError(
                    VirchowErrorCode.INTERNAL_ERROR, "Missing OAuth callback state"
                )
