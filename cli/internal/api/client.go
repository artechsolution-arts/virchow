// Package api provides the HTTP client for communicating with the Virchow server.
package api
...
import (
	...
	"github.com/virchow-dot-app/virchow/cli/internal/models"
)

// Client is the Virchow API client.
type Client struct {
	baseURL        string
	apiKey         string
...
func NewClient(cfg config.VirchowCliConfig) *Client {
...
func (c *Client) UpdateConfig(cfg config.VirchowCliConfig) {
...
func (c *Client) newRequest(ctx context.Context, method, path string, body io.Reader) (*http.Request, error) {
	...
	if c.apiKey != "" {
		...
		req.Header.Set("X-Virchow-Authorization", bearer)
	}
	return req, nil
}
...
func (c *Client) doJSON(ctx context.Context, method, path string, reqBody any, result any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return &VirchowAPIError{StatusCode: resp.StatusCode, Detail: string(respBody)}
	}
...
func (c *Client) TestConnection(ctx context.Context) error {
	...
	if resp2.StatusCode == 401 || resp2.StatusCode == 403 {
		if isHTML || strings.Contains(respServer, "awselb") {
			return fmt.Errorf("HTTP %d from a reverse proxy (not the Virchow backend).\n  Check your deployment's ingress / proxy configuration", resp2.StatusCode)
		}
...
func (c *Client) UploadFile(ctx context.Context, filePath string) (*models.FileDescriptorPayload, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, &VirchowAPIError{StatusCode: resp.StatusCode, Detail: string(body)}
	}
	...
	if len(snapshot.UserFiles) == 0 {
		return nil, &VirchowAPIError{StatusCode: 400, Detail: "File upload returned no files"}
	}
