// Package onboarding handles the first-run setup flow for Virchow CLI.
package onboarding
...
func Run(existing *config.VirchowCliConfig) *config.VirchowCliConfig {
	...
	fmt.Println()
	fmt.Println("  Welcome to " + boldStyle.Render("Virchow CLI") + ".")
	fmt.Println()

	reader := bufio.NewReader(os.Stdin)

	// Server URL
	serverURL := prompt(reader, "  Virchow server URL", cfg.ServerURL)
	if serverURL == "" {
		return nil
	}
	...
	if err := client.TestConnection(context.Background()); err != nil {
		...
		fmt.Println()
		fmt.Println("  " + dimStyle.Render("Run ") + boldStyle.Render("virchow-cli configure") + dimStyle.Render(" to try again."))
		return nil
	}
...
func printQuickStart() {
	...
	fmt.Println()
	fmt.Println("  Just type to chat with your Virchow agent.")
	fmt.Println()
