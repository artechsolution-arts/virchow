...
func writeConfig(t *testing.T, dir string, data []byte) {
	t.Helper()
	virchowDir := filepath.Join(dir, "virchow-cli")
	if err := os.MkdirAll(virchowDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(virchowDir, "config.json"), data, 0o644); err != nil {
		t.Fatal(err)
	}
}
...
func TestDefaultConfig(t *testing.T) {
	cfg := DefaultConfig()
	if cfg.ServerURL != "https://cloud.virchow.app" {
...
func TestLoadFromFile(t *testing.T) {
	...
	data, _ := json.Marshal(map[string]interface{}{
		"server_url":         "https://my-virchow.example.com",
	...
	cfg := Load()
	if cfg.ServerURL != "https://my-virchow.example.com" {
...
func TestSaveCreatesParentDirs(t *testing.T) {
	...
	if err := Save(VirchowCliConfig{APIKey: "test"}); err != nil {
		t.Fatal(err)
	}
