// Custom title bar for Virchow Desktop
// This script injects a draggable title bar that matches Virchow design system

(function () {
  const TITLEBAR_ID = "virchow-desktop-titlebar";
  const TITLEBAR_HEIGHT = 36;
  const STYLE_ID = "virchow-desktop-titlebar-style";
  const VIEWPORT_VAR = "--virchow-desktop-viewport-height";
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        --virchow-desktop-titlebar-height: ${TITLEBAR_HEIGHT}px;
        --virchow-desktop-viewport-height: 100dvh;
        --virchow-desktop-safe-height: calc(var(--virchow-desktop-viewport-height) - var(--virchow-desktop-titlebar-height));
      }

      @supports not (height: 100dvh) {
        :root {
          --virchow-desktop-viewport-height: 100vh;
        }
      }

      html,
      body {
        height: var(--virchow-desktop-viewport-height);
        min-height: var(--virchow-desktop-viewport-height);
        margin: 0;
        padding: 0;
        overflow: hidden;
      }
      ...
      body {
        padding-top: var(--virchow-desktop-titlebar-height) !important;
      ...
      body > div#root,
      body > main {
        height: var(--virchow-desktop-safe-height);
        min-height: var(--virchow-desktop-safe-height);
        overflow: auto;
      }

      /* Override common Tailwind viewport helpers so content fits under the titlebar */
      .h-screen {
        height: var(--virchow-desktop-safe-height) !important;
      }

      .min-h-screen {
        min-height: var(--virchow-desktop-safe-height) !important;
      }

      .max-h-screen {
        max-height: var(--virchow-desktop-safe-height) !important;
      }
      ...
      #${TITLEBAR_ID} {
        ...
        height: var(--virchow-desktop-titlebar-height);
      }
  ...
  function buildTitleBar() {
    ...
    // Apply styles matching Virchow design system with translucent glass effect
