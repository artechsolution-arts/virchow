...
A lightweight macOS desktop application for [Virchow Cloud](https://cloud.virchow.app).
...
- 🔗 **Multi-window** - Open multiple Virchow windows
...
The app defaults to `https://cloud.virchow.app` but supports any Virchow instance.

**Config file location:**

- macOS: `~/Library/Application Support/app.virchow.desktop/config.json`
- Linux: `~/.config/app.virchow.desktop/config.json`
- Windows: `%APPDATA%/app.virchow.desktop/config.json`
...
```json
{
  "server_url": "https://your-virchow-instance.company.com",
  "window_title": "Virchow"
}
```
...
```bash
# macOS
open -t ~/Library/Application\ Support/app.virchow.desktop/config.json

# Or use any editor
code ~/Library/Application\ Support/app.virchow.desktop/config.json
```
...
```json
{
  "app": {
    "windows": [
      {
        "url": "https://your-virchow-instance.com"
      }
    ]
  }
}
...
Make sure you have an internet connection. The app loads content from `cloud.virchow.app`.
...
2. Sign the app: `codesign --deep --force --sign "Developer ID" target/release/bundle/macos/Virchow.app`
