...
const DEFAULT_SERVER_URL: &str = "https://cloud.virchow.app";
const CONFIG_FILE_NAME: &str = "config.json";
#[cfg(target_os = "macos")]
const TITLEBAR_SCRIPT: &str = include_str!("../../src/titlebar.js");
const TRAY_ID: &str = "virchow-tray";
...
(() => {
  if (window.__VIRCHOW_CHAT_LINK_INTERCEPT_INSTALLED__) {
    return;
  }

  window.__VIRCHOW_CHAT_LINK_INTERCEPT_INSTALLED__ = true;
...
(() => {
  if (window.__VIRCHOW_MENU_KEY_HANDLER__) return;
  window.__VIRCHOW_MENU_KEY_HANDLER__ = true;
...
(() => {
  if (window.__VIRCHOW_CONSOLE_CAPTURE__) return;
  window.__VIRCHOW_CONSOLE_CAPTURE__ = true;
...
fn get_config_dir() -> Option<PathBuf> {
    ProjectDirs::from("app", "virchow", "virchow-desktop").map(|dirs| dirs.config_dir().to_path_buf())
}
...
fn is_debug_mode() -> bool {
    std::env::args().any(|arg| arg == "--debug") || std::env::var("VIRCHOW_DEBUG").is_ok()
}
...
fn trigger_new_window(app: &AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let window_label = format!("virchow-{}", uuid::Uuid::new_v4());
...
fn open_docs() {
    let _ = open_in_default_browser("https://docs.virchow.app");
}
...
fn handle_open_debug_log() {
    ...
    if !log_path.exists() {
        eprintln!("[VIRCHOW DEBUG] Log file does not exist yet: {:?}", log_path);
        return;
    }
...
fn setup_app_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    let docs_item = MenuItem::with_id(app, "open_docs", "Virchow Documentation", true, None::<&str>)?;
...
fn build_tray_menu(app: &AppHandle) -> tauri::Result<Menu<Wry>> {
    let open_app = MenuItem::with_id(app, TRAY_MENU_OPEN_APP_ID, "Open Virchow", true, None::<&str>)?;
    ...
    let quit = PredefinedMenuItem::quit(app, Some("Quit Virchow"))?;
...
fn setup_tray_icon(app: &AppHandle) -> tauri::Result<()> {
    let mut builder = TrayIconBuilder::with_id(TRAY_ID).tooltip("Virchow");
...
fn main() {
    ...
    let debug_log_file = if debug_mode {
        eprintln!("[VIRCHOW DEBUG] Debug mode enabled");
        if let Some(path) = get_debug_log_path() {
            eprintln!("[VIRCHOW DEBUG] Frontend logs: {}", path.display());
        }
        eprintln!("[VIRCHOW DEBUG] DevTools will open automatically");
        eprintln!("[VIRCHOW DEBUG] Capturing console.log/warn/error/info/debug from webview");
        init_debug_log_file()
    } else {
        None
    };
