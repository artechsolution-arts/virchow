...
import { getVirchowDomain } from "../utils/storage.js";

(function () {
  let mainIframe = document.getElementById("virchow-iframe");
  ...
  async function preloadChatInterface() {
    ...
    const domain = await getVirchowDomain();
  ...
  function checkVirchowPreference() {
    chrome.storage.local.get(
      ...
      (items) => {
        let useVirchowAsDefaultNewTab =
          items[CHROME_SPECIFIC_STORAGE_KEYS.USE_VIRCHOW_AS_DEFAULT_NEW_TAB];

        if (useVirchowAsDefaultNewTab === undefined) {
          useVirchowAsDefaultNewTab = !!(
        ...
        setIframeSrc(items[CHROME_SPECIFIC_STORAGE_KEYS.VIRCHOW_DOMAIN] + "/nrf");
      },
    );
  }
  ...
  chrome.storage.onChanged.addListener(function (changes, namespace) {
    if (namespace === "local" && changes.useVirchowAsDefaultNewTab) {
      checkVirchowPreference();
    }
  });
  ...
  window.addEventListener("message", function (event) {
    if (event.data.type === CHROME_MESSAGE.SET_DEFAULT_NEW_TAB) {
      chrome.storage.local.set({ useVirchowAsDefaultNewTab: event.data.value });
    } else if (event.data.type === CHROME_MESSAGE.VIRCHOW_APP_LOADED) {
      clearTimeout(iframeLoadTimeout);
      hideErrorModal();
      fadeInContent();
      iframeLoaded = true;
