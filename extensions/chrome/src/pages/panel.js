...
(function () {
  const iframe = document.getElementById("virchow-panel-iframe");
  ...
  // Returns the origin of the Virchow app loaded in the iframe.
  ...
  async function checkPendingInput() {
    try {
      ...
      console.error("[Virchow Panel] Error checking pending input:", error);
    }
    return false;
  }
  ...
  function handleMessage(event) {
    // Only trust messages from the Virchow app iframe.
    ...
    if (event.data.type === CHROME_MESSAGE.VIRCHOW_APP_LOADED) {
  ...
  async function loadVirchowDomain() {
    const response = await chrome.runtime.sendMessage({
      action: ACTIONS.GET_CURRENT_VIRCHOW_DOMAIN,
    });
    if (response && response[CHROME_SPECIFIC_STORAGE_KEYS.VIRCHOW_DOMAIN]) {
      setIframeSrc(
        response[CHROME_SPECIFIC_STORAGE_KEYS.VIRCHOW_DOMAIN] + SIDE_PANEL_PATH,
        "",
      );
    } else {
      console.warn("Virchow domain not found, using default");
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === ACTIONS.OPEN_VIRCHOW_WITH_INPUT) {
