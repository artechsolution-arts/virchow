...
interface MdChunksAskResponse {
  answer?: string;
  sources?: Array<{ doc_id: string; source_file: string; file_id?: string | null }>;
  duration_ms?: number;
}
    ...
    async ({
      ...
      let retrievalType: RetrievalType =
        selectedDocuments.length > 0
          ...
          : RetrievalType.None;
      let documents: VirchowDocument[] = selectedDocuments;
      ...
      if (useMdChunksChat) {
        ...
        try {
          const response = await fetch("/api/md-chunks-ask", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          if (!response.ok) {
            throw new Error(`md_chunks endpoint failed: ${response.status}`);
          }
          ...
          const sourcesBlock =
            sources.length > 0
              ? `\n\nSources:\n${sources
                  .map((source) => {
                    return `- [${source.source_file}](/md-chunks-source/${encodeURIComponent(
                      source.doc_id
          ...
          const timingLine =
            typeof data.duration_ms === "number"
              ? `\n\n*(${data.duration_ms} ms total)*`
              : "";
          const mdChunksAnswer = `${data.answer || "No answer returned."}${sourcesBlock}${timingLine}`;
          ...
          console.error("md_chunks chat path failed:", error);
          const errMsg =
          ...
          upsertToCompleteMessageTree({
            messages: [
              ...
              },
              {
                ...initialAgentNode,
                message:
                  errMsg.includes("aborted") || errMsg.includes("signal")
                    ? "Answer timed out. Try a shorter question or check Postgres/OpenAI."
                    : `Could not get answer: ${errMsg}`,
                type: "error",
                files: [],
                toolCall: null,
                parentNodeId: initialUserNode.nodeId,
