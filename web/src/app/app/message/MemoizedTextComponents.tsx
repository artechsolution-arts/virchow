...
import {
  LoadedVirchowDocument,
  MinimalVirchowDocument,
  VirchowDocument,
...
export const MemoizedAnchor = memo(
  ({
    ...
    docs?: VirchowDocument[] | null;
    userFiles?: ProjectFile[] | null;
    citations?: CitationMap;
    updatePresentingDocument: (doc: MinimalVirchowDocument) => void;
    href?: string;
    ...
    if (value?.startsWith("[") && value?.endsWith("]")) {
      ...
      if (match) {
        ...
        if (match_item !== undefined) {
          ...
          let associatedDoc: VirchowDocument | null = null;
...
export const MemoizedLink = memo(
  ({
    ...
    const documentSourceInfo = useMemo(() => {
      ...
      return documentToSourceInfo(document as VirchowDocument);
    }, [document]);
    ...
    const handleSourceClick = useCallback(() => {
      if (document && updatePresentingDocument) {
        openDocument(document as VirchowDocument, updatePresentingDocument);
    ...
    if (value?.toString().startsWith("*")) {
      ...
      const displayName = document
        ? getDisplayNameForSource(document as VirchowDocument)
