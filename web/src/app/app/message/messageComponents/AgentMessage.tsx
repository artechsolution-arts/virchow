...
export interface AgentMessageProps {
  ...
  /** When set with empty packets (e.g. md_chunks path), render as final answer markdown */
  staticAnswerMarkdown?: string;
}
...
function arePropsEqual(
  ...
  return (
    ...
    prev.staticAnswerMarkdown === next.staticAnswerMarkdown
...
const AgentMessage = React.memo(function AgentMessage({
  ...
  const trimmedStaticAnswer = (staticAnswerMarkdown ?? "").trim();
  const showStaticAnswerOnly =
    trimmedStaticAnswer.length > 0 && rawPackets.length === 0;

  const { renderedContent: staticRenderedMarkdown } = useMarkdownRenderer(
    showStaticAnswerOnly ? trimmedStaticAnswer : "",
    effectiveChatState,
  ...
  const messageCompleteForUi = isComplete || showStaticAnswerOnly;

  return (
    <div
      className="flex flex-col gap-3"
      data-testid={messageCompleteForUi ? "virchow-ai-message" : undefined}
    >
      ...
      <AgentTimeline
        ...
        hasDisplayContent={
          pacedDisplayGroups.length > 0 || showStaticAnswerOnly
        }
      ...
      >
        {(pacedDisplayGroups.length > 0 || showStaticAnswerOnly) && (
          <div ref={finalAnswerRef} className="flex flex-col gap-3">
            ...
            {showStaticAnswerOnly ? (
              <div className="select-text">{staticRenderedMarkdown}</div>
            ) : null}
          </div>
        )}
