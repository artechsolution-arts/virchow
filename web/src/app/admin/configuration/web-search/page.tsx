...
export default function Page() {
  ...
  const combinedContentProviders = useMemo(() => {
    ...
    const ordered = CONTENT_PROVIDER_ORDER.map((providerType) => {
      ...
      if (providerType === "virchow_web_crawler") {
        return {
          id: -1,
          name: "Virchow Web Crawler",
          provider_type: "virchow_web_crawler",
          is_active: true,
          config: null,
          has_api_key: true,
  ...
  const handleActivateContentProvider = async (
    ...
    try {
      if (provider.provider_type === "virchow_web_crawler") {
        const response = await fetch(
  ...
  const handleDeactivateContentProvider = async (
    ...
    try {
      // For virchow_web_crawler (virtual provider with id -1), use reset-default
      // For real providers, use the deactivate endpoint
      const endpoint =
        providerType === "virchow_web_crawler" || providerId < 0
          ? "/api/admin/web-search/content-providers/reset-default"
  ...
  const handleContentConnect = async () => {
    ...
    // Note: virchow_web_crawler doesn't go through this modal.
    ...
    <>
            ...
            <div className="flex flex-col gap-2">
              {combinedContentProviders.map((provider) => {
                ...
                const buttonState = (() => {
                  ...
                  const canActivate =
                    providerId > 0 ||
                    provider.provider_type === "virchow_web_crawler" ||
                    isConfigured;
      ...
      <WebProviderSetupModal
        ...
        providerLogo={renderLogo({
          ...
          fallback:
            selectedContentProviderType === "virchow_web_crawler" ? (
              <SvgVirchowLogo size={24} className="text-text-05" />
            ) : undefined,
          size: 24,
          containerSize: 28,
        })}
