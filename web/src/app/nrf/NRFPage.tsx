...
export default function NRFPage({ isSidePanel = false }: NRFPageProps) {
  const { setUseVirchowAsNewTab: setUseVirchowAsNewTab } = useNRFPreferences();
  ...
  // If user toggles the "Use Virchow" switch to off, prompt a modal
  const handleUseVirchowToggle = (checked: boolean) => {
    if (!checked) {
      setShowTurnOffModal(true);
    } else {
      setUseVirchowAsNewTab(true);
      sendSetDefaultNewTabMessage(true);
    }
  };

  const confirmTurnOff = () => {
    setUseVirchowAsNewTab(false);
      ...
      {!isSidePanel && (
        <>
          <SettingsPanel
            ...
            handleUseVirchowToggle={handleUseVirchowToggle}
          />
          ...
          <Modal open={showTurnOffModal} onOpenChange={setShowTurnOffModal}>
            <Modal.Content width="sm">
              <Modal.Header
                ...
                title="Turn off Virchow new tab page?"
                description="You'll see your browser's default new tab page instead. You can turn it back on anytime in your Virchow settings."
      ...
      {!user && (
        <Modal open onOpenChange={() => {}}>
          <Modal.Content width="sm" height="sm">
            <Modal.Header icon={SvgUser} title="Welcome to Virchow" />
            <Modal.Body>
