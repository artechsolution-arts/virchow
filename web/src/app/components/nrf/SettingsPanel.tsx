...
export const SettingsPanel = ({
  settingsOpen,
  toggleSettings,
  handleUseVirchowToggle,
}: {
  settingsOpen: boolean;
  toggleSettings: () => void;
  handleUseVirchowToggle: (checked: boolean) => void;
}) => {
  const { useVirchowAsNewTab } = useNRFPreferences();
        ...
        <div className="px-6 pb-8 flex flex-col gap-8">
          ...
          <section className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1 bg-background-tint-01 rounded-2xl px-4">
              <SettingRow label="Use Virchow as new tab page">
                <Switch
                  checked={useVirchowAsNewTab}
                  onCheckedChange={handleUseVirchowToggle}
                />
              </SettingRow>
            </div>
          </section>
