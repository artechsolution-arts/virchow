...
import { VirchowBotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useVirchowBotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/virchowbot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<VirchowBotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshVirchowBotAnalytics: () => mutate(url),
  };
};
