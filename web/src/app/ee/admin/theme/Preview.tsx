...
import { VirchowIcon } from "@/components/icons/icons";
...
function PreviewLogo({
  ...
  logoSrc?: string;
  forceVirchowIcon?: boolean;
  size: number;
  className?: string;
}) {
  return logoSrc && !forceVirchowIcon ? (
    <img
      src={logoSrc}
      alt="Logo"
      style={{
    ...
    <VirchowIcon size={size} className={cn("flex-shrink-0", className)} />
  );
}
          ...
          >
            ...
            {(logoDisplayStyle === "logo_and_name" ||
              ...
              <Truncated mainUiAction text04 nowrap>
                {applicationDisplayName || "Virchow"}
              </Truncated>
            )}
          </div>
        </div>
