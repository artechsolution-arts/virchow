...
import VirchowInitializingLoader from "@/components/VirchowInitializingLoader";
import { VirchowDocument, MinimalVirchowDocument } from "@/lib/search/interfaces";
import {
  useSettingsContext,
  useVectorDbEnabled,
...
import { FileUploadContent } from "@/refresh-pages/admin/FileUploadPage";
...
export default function AppPage({ firstMessage }: ChatPageProps) {
  ...
  const activeAgent: MinimalPersonaSnapshot = liveAgent ?? {
    id: 0,
    name: "Ollama LLM",
  ...
  const [presentingDocument, setPresentingDocument] =
    useState<MinimalVirchowDocument | null>(null);
  ...
  const sources: SourceMetadata[] = useMemo(() => {
    ...
    const federatedSources =
      (Array.isArray(federatedConnectorsData) ? federatedConnectorsData : []).map(
        (connector: FederatedConnectorDetail) => {
  ...
  const [selectedDocuments, setSelectedDocuments] = useState<VirchowDocument[]>(
    []
  );
  ...
  const handleSearchDocumentClick = useCallback(
    (doc: MinimalVirchowDocument) => setPresentingDocument(doc),
    []
  );
  ...
  // REMOVED NoAgentModal check per user instructions
  ...
  if (!isReady) return <VirchowInitializingLoader />;
            ...
            >
              ...
              >
                ...
                <div className="row-start-1 min-h-0 overflow-hidden flex flex-col items-center">
                  ...
                  <Fade
                    show={
                      appFocus.isChat() && !!currentChatSessionId && !!activeAgent
                    }
