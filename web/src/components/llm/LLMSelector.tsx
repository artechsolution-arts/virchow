...
export default function LLMSelector({
  ...
  const safeProviders = Array.isArray(llmProviders) ? llmProviders : [];
  ...
  const llmOptions = useMemo(() => {
    ...
    safeProviders.forEach((provider) => {
      const modelConfigurations = Array.isArray(provider.model_configurations)
        ? provider.model_configurations
        : [];

      modelConfigurations.forEach((modelConfiguration) => {
  ...
  const defaultProvider = defaultText
    ? safeProviders.find((p) => p.id === defaultText.provider_id)
    : undefined;
