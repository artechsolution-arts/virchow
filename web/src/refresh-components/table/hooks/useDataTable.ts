...
export type VirchowSortDirection = "none" | "ascending" | "descending";
export type VirchowSelectionState = "none" | "partial" | "all";
...
/**
 * Convert a TanStack sort direction to an Virchow sort direction string.
 *
 ...
 */
export function toVirchowSortDirection(
  dir: false | "asc" | "desc"
): VirchowSortDirection {
...
interface UseDataTableReturn<TData extends RowData> {
  ...
  // Pagination (1-based, matching Virchow Footer)
  ...
  // Selection (pre-computed for Virchow Footer)
  /** Aggregate selection state for the current page. */
  selectionState: VirchowSelectionState;
...
/**
 * Wraps TanStack `useReactTable` with Virchow-specific defaults and derived
...
export default function useDataTable<TData extends RowData>(
  ...
  const selectionState: VirchowSelectionState = isAllPageRowsSelected
    ? "all"
