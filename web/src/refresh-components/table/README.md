...
`createTableColumns<TData>()` returns a typed builder with four methods. Each returns an `VirchowColumnDef<TData>` that you pass to the `columns` prop.
...
| `columns`                 | `VirchowColumnDef<TData>[]`          | **required**                                  | Columns from `createTableColumns()`                                                                    |
