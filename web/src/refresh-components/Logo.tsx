...
import { VirchowIcon, VirchowLogoTypeIcon } from "@/components/icons/icons";
...
export default function Logo({ folded, size, className }: LogoProps) {
  ...
  return applicationName ? (
    ...
    <VirchowIcon size={foldedSize} className={cn("flex-shrink-0", className)} />
  ) : (
    <VirchowLogoTypeIcon size={unfoldedSize} className={className} />
  );
}
