...
function buildItems(
  ...
  if (vectorDbEnabled) {
    ...
    add(SECTIONS.DOCUMENTS_AND_KNOWLEDGE, ADMIN_ROUTES.UPLOAD_FILES);
  ...
  if (!isCurator) {
    add(SECTIONS.ORGANIZATION, ADMIN_ROUTES.LLM_MODELS);
  }

  // 4. Usage
  add(SECTIONS.USAGE, ADMIN_ROUTES.QUERY_HISTORY);
  if (customAnalyticsEnabled) {
    add(SECTIONS.USAGE, ADMIN_ROUTES.CUSTOM_ANALYTICS);
  }
      ...
      <SidebarBody
        ...
        footer={
          <Section gap={0} height="fit" alignItems="start">
            ...
            {/* Footer version information removed */}
          </Section>
        }
      >
