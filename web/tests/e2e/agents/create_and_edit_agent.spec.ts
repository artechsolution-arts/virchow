...
test.describe("Assistant Creation and Edit Verification", () => {
  ...
  test.describe("User Files Only", () => {
    ...
    test.afterAll(async ({ browser }: { browser: Browser }) => {
      if (userFilesAssistantId !== null) {
        ...
        const cleanupClient = new VirchowApiClient(page.request);
  ...
  test.describe("With Knowledge", () => {
    ...
    test("should create and edit assistant with Knowledge enabled", async ({
      ...
      const virchowApiClient = new VirchowApiClient(page.request);
      ccPairId = await virchowApiClient.createFileConnector("Test Connector");
      documentSetId = await virchowApiClient.createDocumentSet(
        "Test Document Set",
        [ccPairId]
      );
