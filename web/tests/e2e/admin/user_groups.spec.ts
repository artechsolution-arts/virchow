...
test.describe("User Groups - No Vector DB @lite", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      const client = new VirchowApiClient(context.request);
  ...
  test("should show user group as synced immediately on creation", async ({
    ...
    try {
      ...
      await expect(
        dialog.getByText("Connectors are not available in Virchow Lite")
      ).toBeVisible();
...
test.describe("User Groups - Standard Deployment @exclusive", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      client = new VirchowApiClient(context.request);
      const vectorDbEnabled = await client.isVectorDbEnabled();
      if (!vectorDbEnabled) {
        await context.close();
