...
test.describe("Web Content Provider Configuration", () => {
  ...
  test.describe("Firecrawl Provider", () => {
    ...
    test("should switch back to Virchow Web Crawler from Firecrawl", async ({
      page,
    }) => {
      ...
      console.log(
        "[web-content-test] Firecrawl configured, now switching to Virchow Web Crawler..."
      );

      // Switch to Virchow Web Crawler
      const virchowCrawlerCard = await findProviderCard(page, "Virchow Web Crawler");
      await virchowCrawlerCard.waitFor({ state: "visible", timeout: 10000 });

      const virchowSetDefault = virchowCrawlerCard.getByRole("button", {
        name: "Set as Default",
      });

      if (await virchowSetDefault.isVisible()) {
        await virchowSetDefault.click();
        await page.waitForLoadState("networkidle");
      }

      await expect(
        virchowCrawlerCard.getByRole("button", { name: "Current Crawler" })
      ).toBeVisible({ timeout: 15000 });

      console.log("[web-content-test] Switched back to Virchow Web Crawler");
    });
  });
});
