...
import type { VirchowApiClient } from "@tests/e2e/utils/virchowApiClient";
...
function uniqueEmail(prefix: string): string {
  return `e2e-${prefix}-${Date.now()}@test.virchow`;
}
...
async function withApiContext(
  browser: Browser,
  fn: (api: VirchowApiClient) => Promise<void>
): Promise<void> {
  const context = await browser.newContext({
    storageState: "admin_auth.json",
  });
  try {
    const { VirchowApiClient } = await import("@tests/e2e/utils/virchowApiClient");
    const api = new VirchowApiClient(context.request);
    await fn(api);
  } finally {
    await context.close();
  }
