...
test.describe("Default Agent Tests", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const apiClient = new VirchowApiClient(adminPage.request);
    try {
  ...
  test.describe("Greeting Message Display", () => {
    ...
    test("greeting should only appear for default agent", async ({ page }) => {
      ...
      const customGreeting = await page.$('[data-testid="virchow-logo"]');
      expect(customGreeting).toBeNull();
    });
  });

  test.describe("Default Agent Branding", () => {
    test("should display Virchow logo for default agent", async ({ page }) => {
      // Look for Virchow logo
    ...
    test("custom agents should show name and icon instead of logo", async ({
      ...
      // Should show agent name and icon, not Virchow logo
      ...
      // Virchow logo should NOT be shown
      const logoElement = await page.$('[data-testid="virchow-logo"]');
  ...
  test.describe("Agent Selection", () => {
    test("default agent should be selected for new chats", async ({ page }) => {
      // Verify the input placeholder indicates default agent (Virchow)
...
test.describe("End-to-End Default Agent Flow", () => {
  ...
  test("complete user journey with default agent", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="virchow-logo"]')).toBeVisible();

    // Verify Virchow logo is displayed
    await expect(page.locator('[data-testid="virchow-logo"]')).toBeVisible();
