...
export async function verifyDefaultAgentIsChosen(page: Page) {
  await expect(page.getByTestId("virchow-logo")).toBeVisible({ timeout: 5000 });
}
...
export async function sendMessage(page: Page, message: string) {
  ...
  const existingMessageCount = await page
    .locator('[data-testid="virchow-ai-message"]')
    .count();

  await page.locator("#virchow-chat-input-textarea").click();
  await page.locator("#virchow-chat-input-textarea").fill(message);
  await page.locator("#virchow-chat-input-send-button").click();

  // Wait for a NEW AI message to appear (count should increase)
  await expect(page.locator('[data-testid="virchow-ai-message"]')).toHaveCount(
