...
/**
 * API Client for Virchow backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the Virchow backend API.
 ...
 * const client = new VirchowApiClient(page.request);
 *
 ...
 * const client = new VirchowApiClient(ctx, baseURL);
 * ```
 *
...
export class VirchowApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[VirchowApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[VirchowApiClient]")
   */
  private log(message: string): void {
    console.log(`[VirchowApiClient] ${message}`);
  }
