...
const nextConfig = {
    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            // Omit deprecated/experimental tokens that Chrome logs as "Unrecognized feature"
            value:
              "accelerometer=(), autoplay=(), camera=(), cross-origin-isolated=(), display-capture=(), encrypted-media=(), fullscreen=(), geolocation=(), gyroscope=(), keyboard-map=(), magnetometer=(), microphone=(self), midi=(), payment=(), picture-in-picture=(), publickey-credentials-get=(), screen-wake-lock=(), sync-xhr=(), usb=(), web-share=(), xr-spatial-tracking=()",
          },
        ],
      },
      {
  ...
  async rewrites() {
    return [
      ...
      },
      {
        ...
        destination: `${
          process.env.INTERNAL_URL || "http://localhost:8081"
        }/docs/:path*`,
      },
      {
  ...
  async redirects() {
    return [
      ...
      // TODO: Remove these redirects in v4.0 — https://linear.app/virchow-app/issue/ENG-3771
      {
...
const sentryWebpackPluginOptions = {
  org: process.env.SENTRY_ORG || "virchow-vl",
  project: process.env.SENTRY_PROJECT || "virchow-web",
