# Virchow Developer Script

[![Deploy Status](https://github.com/virchow-dot-app/virchow/actions/workflows/release-devtools.yml/badge.svg)](https://github.com/virchow-dot-app/virchow/actions/workflows/release-devtools.yml)
[![PyPI](https://img.shields.io/pypi/v/virchow-devtools.svg)](https://pypi.org/project/virchow-devtools/)

`ods` is [virchow.app](https://github.com/virchow-dot-app/virchow)'s devtools utility script.
It is packaged as a python [wheel](https://packaging.python.org/en/latest/discussions/package-formats/) and available from [PyPI](https://pypi.org/project/virchow-devtools/).

## Installation

A stable version of `ods` is provided in the default [python venv](https://github.com/virchow-dot-app/virchow/blob/main/CONTRIBUTING.md#backend-python-requirements)
which is synced automatically if you have [pre-commit](https://github.com/virchow-dot-app/virchow/blob/main/CONTRIBUTING.md#formatting-and-linting)
hooks installed.

While inside the Virchow repository, activate the root project's venv,
...
Launch Virchow docker containers using docker compose.
...
View logs from running Virchow docker containers. Service names are available as
...
Pull the latest images for Virchow docker containers.
...
- `api` - Start the FastAPI backend server (`uvicorn virchow.main:app --reload`)
...
| Flag | Default |
|------|---------|
| `--baseline` | `s3://virchow-playwright-artifacts/baselines/<project>/<rev>/` |
| `--current` | `web/output/screenshots/` |
| `--output` | `web/output/screenshot-diff/<project>/index.html` |
| `--rev` | `main` |

The S3 bucket defaults to `virchow-playwright-artifacts` and can be overridden with the
...
To upgrade the stable version, upgrade it as you would any other [requirement](https://github.com/virchow-dot-app/virchow/tree/main/backend/requirements#readme).
...
with the pip version of `ods` installed in the Virchow venv._
...
Releases are deployed automatically when git tags prefaced with `ods/` are pushed to [GitHub](https://github.com/virchow-dot-app/virchow/tags).
...
See also, [`.github/workflows/release-devtools.yml`](https://github.com/virchow-dot-app/virchow/blob/main/.github/workflows/release-devtools.yml).
