...
snapshots directory (~/.local/share/virchow-dev/snapshots/) if no file is specified.

Examples:
  ods db dump                           # Creates virchow_<timestamp>.dump in snapshots dir
...
func runDBDump(opts *DBDumpOptions) {
	...
	containerTmpFile := "/tmp/virchow_dump_tmp"
...
func determineOutputPath(output string, format string) string {
	...
	if output == "" {
		...
		filename := fmt.Sprintf("virchow_%s%s", timestamp, ext)
