...
func TestLitellmSingletonEagerImportDetection(t *testing.T) {
	...
	testContent := `
import os
from virchow.llm.litellm_singleton import litellm  # Should be flagged as eager import
from typing import Dict
...
`
	...
	patterns := createPatterns([]string{"virchow.llm.litellm_singleton"})
	result := findEagerImports(testPath, patterns)
	...
	if _, ok := result.ViolatedModules["virchow.llm.litellm_singleton"]; !ok {
		t.Error("Expected virchow.llm.litellm_singleton in violated modules")
	}
...
func TestDefaultLazyImportModules(t *testing.T) {
	...
	expectedLitellmIgnores := []string{
		"virchow/llm/litellm_singleton/__init__.py",
		"virchow/llm/litellm_singleton/config.py",
		"virchow/llm/litellm_singleton/monkey_patches.py",
	}
...
func TestIsValidPythonFile(t *testing.T) {
	tests := []struct {
		...
		{"regular python file", "virchow/main.py", true},
		{"non-python file", "virchow/main.go", false},
		{"test file prefix", "virchow/test_main.py", false},
		{"test file suffix", "virchow/main_test.py", false},
		...
		{"regular in subdir", "virchow/connectors/file.py", true},
	}
...
func TestShouldCheckFileForModule(t *testing.T) {
	...
	// Create test file
	testFile := filepath.Join(tmpDir, "virchow", "llm", "test.py")
	...
	settings = NewLazyImportSettings("virchow/llm/test.py")
	if shouldCheckFileForModule(testFile, tmpDir, settings) {
