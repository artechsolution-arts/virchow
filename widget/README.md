...
```html
<!-- Load the widget -->
<script type="module" src="https://cdn.virchow.app/widget/1.0/dist/virchow-widget.js"></script>

<!-- Configure and display -->
<virchow-chat-widget
  backend-url="https://cloud.virchow.app/api"
...
| ------------- | ------ | -------------------------------------------------------------------- |
| `backend-url` | string | Your Virchow backend API URL (or set `VITE_WIDGET_BACKEND_URL` in .env) |
...
| `logo`             | string | Virchow logo     | URL to custom logo image                 |
...
```html
<virchow-chat-widget backend-url="https://cloud.virchow.app/api" api-key="on_abc123">
</virchow-chat-widget>
```
...
```html
<virchow-chat-widget mode="launcher"></virchow-chat-widget>
```
...
<div class="chat-container">
  <virchow-chat-widget mode="inline"></virchow-chat-widget>
</div>
```
...
│   └── assets/
│       └── logo.ts              # Default Virchow logo (base64)
├── dist/                        # Build output
├── index.html
├── package.json
├── vite.config.ts
...
The widget communicates with these Virchow backend endpoints:
...
3. **Deploy `dist/virchow-widget.js` to your server**

4. **Customer embed:**
   ```html
   <script type="module" src="https://your-cdn.com/virchow-widget.js"></script>
   <virchow-chat-widget
     agent-id="1"
     agent-name="Support"
...
```css
--theme-primary-05: #1c1c1c; /* Buttons, accents (virchow-ink-95) */
--theme-primary-06: #000000; /* Hover state (virchow-ink-100) */
