...
* cd charts/virchow
* helm install virchow . -n virchow --set postgresql.primary.persistence.enabled=false --set auth.opensearch.values.opensearch_admin_password='StrongPassword123!'
...
* kubectl -n virchow port-forward service/virchow-nginx 8080:80
  * this will forward the local port 8080 to the installed chart for you to run tests, etc.
* When you are finished
  * helm uninstall virchow -n virchow
  * Vespa leaves behind a PVC. Delete it if you are completely done.
    * k -n virchow get pvc
    * k -n virchow delete pvc vespa-storage-da-vespa-0
  * If you didn't disable Postgres persistence earlier, you may want to delete that PVC too.

## Run as non-root user
By default, some virchow containers run as root. If you'd like to explicitly run the virchow containers as a non-root user, update the values.yaml file for the following components:
...
## Resourcing
In the helm charts, we have resource suggestions for all Virchow-owned components. 
...
1. Install and manage the KEDA operator in your cluster yourself (for example via the official KEDA Helm chart). KEDA is no longer packaged as a dependency of the Virchow chart.
