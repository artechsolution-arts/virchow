...
## Overview
This directory contains Terraform modules to provision the core AWS infrastructure for Virchow:
...
- `virchow`: A higher-level composition that wires the above modules together for a complete, opinionated stack

Use the `virchow` module if you want a working EKS + Postgres + Redis + S3 stack with sane defaults. Use the individual modules if you need more granular control.
...
- Provisions the full Virchow AWS stack via the `virchow` module
...
module "virchow" {
  ...
  # source = "./modules/aws/virchow"
  # If referencing from this repo as a template, adjust the path accordingly.
  source = "./modules/aws/virchow"

  region            = local.region
  name              = "virchow"            # used as a prefix and workspace-aware
...
resource "null_resource" "wait_for_cluster" {
  provisioner "local-exec" {
    command = "aws eks wait cluster-active --name ${module.virchow.cluster_name} --region ${local.region}"
  }
}

data "aws_eks_cluster" "eks" {
  name       = module.virchow.cluster_name
...
output "cluster_name" {
  value = module.virchow.cluster_name
}
output "postgres_connection_url" {
  value     = module.virchow.postgres_connection_url
  sensitive = true
}
output "redis_connection_url" {
  value     = module.virchow.redis_connection_url
  sensitive = true
}
```
...
Inputs (common):
- `name` (default `virchow`), `region` (default `us-west-2`), `tags`
...
## Installing the Virchow Helm chart (after Terraform)
Once the cluster is active, deploy application workloads via Helm. You can use the chart in `deployment/helm/charts/virchow`.
...
kubectl create namespace virchow --dry-run=client -o yaml | kubectl apply -f -

# If using AWS S3 via IRSA created by the EKS module, consider disabling MinIO
# Replace the path below with the absolute or correct relative path to the virchow Helm chart
helm upgrade --install virchow /path/to/virchow/deployment/helm/charts/virchow \
  --namespace virchow \
  --set minio.enabled=false \
  --set serviceAccount.create=false \
  --set serviceAccount.name=virchow-s3-access
```

Notes:
- The EKS module can create an IRSA role plus a Kubernetes `ServiceAccount` named `virchow-s3-access` (by default in namespace `virchow`) when `s3_bucket_names` is provided. Use that service account in the Helm chart to avoid static S3 credentials.
...
- Use Terraform workspaces to create isolated environments; the `virchow` module automatically includes the workspace in resource names.
