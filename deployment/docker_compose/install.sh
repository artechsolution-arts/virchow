...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --verbose)
            ...
            ;;
        --help|-h)
            echo "Virchow Installation Script"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --include-craft  Enable Virchow Craft (AI-powered web app building)"
            echo "  --lite           Deploy Virchow Lite (no Vespa, Redis, or model servers)"
            echo "  --local          Use existing config files instead of downloading from GitHub"
            echo "  --shutdown       Stop (pause) Virchow containers"
            echo "  --delete-data    Remove all Virchow data (containers, volumes, and files)"
            ...
            echo ""
            echo "Examples:"
            echo "  $0                    # Install Virchow"
            echo "  $0 --lite             # Install Virchow Lite (minimal deployment)"
            echo "  $0 --include-craft    # Install Virchow with Craft enabled"
            echo "  $0 --shutdown         # Pause Virchow services"
            echo "  $0 --delete-data      # Completely remove Virchow and all data"
...
INSTALL_ROOT="${INSTALL_PREFIX:-virchow_data}"

LITE_COMPOSE_FILE="docker-compose.virchow-lite.yml"
...
if [ "$SHUTDOWN_MODE" = true ]; then
    echo ""
    echo -e "${BLUE}${BOLD}=== Shutting down Virchow ===${NC}"
    echo ""
    
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        print_info "Stopping Virchow containers..."
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "Virchow containers stopped (paused)"
            else
        ...
        print_warning "Virchow data directory not found. Nothing to shutdown."
    fi

    echo ""
    print_success "Virchow shutdown complete!"
    exit 0
fi
...
if [ "$DELETE_DATA_MODE" = true ]; then
    echo ""
    echo -e "${RED}${BOLD}=== WARNING: This will permanently delete all Virchow data ===${NC}"
    echo ""
    print_warning "This action will remove:"
    echo "  • All Virchow containers and volumes"
    ...
    print_info "Removing Virchow containers and volumes..."
    ...
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "Virchow containers and volumes removed"
            else
    ...
    echo ""
    print_success "All Virchow data has been permanently deleted!"
    exit 0
fi
...
if command -v docker &> /dev/null \
    ...
    exec sg docker -c "_VIRCHOW_REEXEC=1 bash $(printf '%q ' "$0" "$@")"
fi
...
echo "              |___/       "
echo -e "${NC}"
echo "Welcome to Virchow Installation Script"
echo "===================================="
echo ""

# User acknowledgment section
echo -e "${YELLOW}${BOLD}This script will:${NC}"
echo "1. Download deployment files for Virchow into a new '${INSTALL_ROOT}' directory"
...
GITHUB_RAW_URL="https://raw.githubusercontent.com/virchow-dot-app/virchow/main/deployment/docker_compose"
...
if [ "$RESOURCE_WARNING" = true ]; then
    echo ""
    print_warning "Virchow recommends at least ${EXPECTED_DOCKER_RAM_GB}GB RAM and ${EXPECTED_DISK_GB}GB disk space for optimal performance in standard mode."
...
NGINX_BASE_URL="https://raw.githubusercontent.com/virchow-dot-app/virchow/main/deployment/data/nginx"

if [[ "$USE_LOCAL_FILES" = true ]]; then
    print_step "Verifying existing configuration files"
else
    print_step "Downloading Virchow configuration files"
...
if [ -d "${INSTALL_ROOT}/deployment" ] && [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
    ...
    if [ -n "$COMPOSE_CMD" ]; then
        ...
        if [ "$RUNNING_CONTAINERS" -gt 0 ]; then
            print_error "Virchow services are currently running!"
            echo ""
            print_info "To make configuration changes, you must first shut down the services."
            echo ""
            print_info "Please run the following command to shut down Virchow:"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ "$INCLUDE_CRAFT" = true ] || [[ "$VERSION" == craft-* ]]; then
        ...
        sed -i.bak 's/^#* *ENABLE_CRAFT=.*/ENABLE_CRAFT=true/' "$ENV_FILE" 2>/dev/null || true
        print_success "Virchow Craft enabled (ENABLE_CRAFT=true)"
    else
        print_info "Virchow Craft disabled (use --include-craft to enable)"
    fi
    ...
    echo "  • Virchow Craft (set ENABLE_CRAFT=true)"
    echo ""
fi
...
# Start services
print_step "Starting Virchow services"
...
if [ $? -ne 0 ]; then
    print_error "Failed to start Virchow services"
    exit 1
fi
...
check_virchow_health() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        printf "\r\033[KChecking Virchow service%s (%dm %ds elapsed)" "$dots" "$minutes" "$seconds"
...
# Run health check
if check_virchow_health; then
    echo ""
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}${BOLD}   🎉 Virchow service is ready! 🎉${NC}"
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
else
    ...
    echo -e "${YELLOW}${BOLD}   ⚠️  Virchow containers are running ⚠️${NC}"
    echo -e "${YELLOW}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
fi
echo ""
print_info "Access Virchow at:"
...
echo ""
print_info "For help or issues, contact: founders@virchow.app"
echo ""