...
To set up Virchow there are several options, Virchow supports the following for deployment:
...
**For more detailed guides, please refer to the documentation: https://docs.virchow.app/deployment/overview**

## install.sh script

```
curl -fsSL https://raw.githubusercontent.com/virchow-dot-app/virchow/main/deployment/docker_compose/install.sh > install.sh && chmod +x install.sh && ./install.sh
```

This provides a guided installation of Virchow via Docker Compose. It will deploy the latest version of Virchow
and set up the volumes to ensure data is persisted across deployments or upgrades.

The script will create an virchow_data directory, all necessary files for the deployment will be stored in
...
Virchow maintains backwards compatibility across all minor versions following SemVer. If following the install.sh script (or through Docker Compose), you can
...
IMAGE_TAG is the version of Virchow to run. It is recommended to leave it as latest to get all updates with each redeployment.
