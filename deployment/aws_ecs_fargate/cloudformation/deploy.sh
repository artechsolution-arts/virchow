...
# Unified config file
CONFIG_FILE="virchow_config.jsonl"
...
if [ -n "$S3_BUCKET_FROM_CONFIG" ]; then
    ...
    S3_BUCKET="${S3_BUCKET:-virchow-ecs-fargate-configs}"
fi

INFRA_ORDER=(
  "virchow_efs_template.yaml"
  "virchow_cluster_template.yaml"
  "virchow_acm_template.yaml"
)

# Deployment order for services
SERVICE_ORDER=(
  "virchow_postgres_service_template.yaml"
  "virchow_redis_service_template.yaml"
  "virchow_vespaengine_service_template.yaml"
  "virchow_model_server_indexing_service_template.yaml"
  "virchow_model_server_inference_service_template.yaml"
  "virchow_backend_api_server_service_template.yaml"
  "virchow_backend_background_server_service_template.yaml"
  "virchow_web_server_service_template.yaml"
  "virchow_nginx_service_template.yaml"
)
...
deploy_infra_stacks() {
    for template_name in "${INFRA_ORDER[@]}"; do
      ...
      if [[ "$template_name" == "virchow_acm_template.yaml" ]]; then
...
echo "Starting deployment of Virchow to ECS Fargate Cluster..."
deploy_infra_stacks
deploy_services_stacks
