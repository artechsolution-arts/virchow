...
- You should assume that all Virchow services are running. To verify, you can check the `backend/log` directory to
  make sure we see logs coming out from the relevant service.
- To connect to the Postgres database, use: `docker exec -it virchow-relational_db-1 psql -U postgres -c "<SQL>"`
- When making calls to the backend, always go through the frontend. E.g. make a call to `http://localhost:3000/api/persona` not `http://localhost:8080/api/persona`
- Put ALL db operations under the `backend/virchow/db` / `backend/ee/virchow/db` directories. Don't run queries
  outside of those directories.

## Project Overview

**Virchow** (formerly Danswer) is an open-source Gen-AI and Enterprise Search platform that connects to company documents, apps, and people. It features a modular architecture with both Community Edition (MIT licensed) and Enterprise Edition offerings.

### Background Workers (Celery)

Virchow uses Celery for asynchronous task processing with multiple specialized workers:
...
**Reason:** We have a very carefully curated selection of icons that match our Virchow guidelines. We do NOT want to muddy those up with different aesthetic stylings.
...
If you need help with this step, reach out to `raunak@virchow.app`.
...
**Reason:** We've put in a lot of effort to unify the components that are rendered in the Virchow app. Using raw components breaks the entire UI of the application, and leaves it in a muddier state than before.
...
```typescript
// ✅ Good - Use custom Virchow color classes
...
There are 4 main types of tests within Virchow:

### Unit Tests

These should not assume any Virchow/external services are available to be called.
...
These tests assume that all external dependencies of Virchow are available and callable (e.g. Postgres, Redis,
MinIO/S3, Vespa are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual Virchow containers are not running and with these tests we call the function to test directly.
...
Standard integration tests. Every test in `backend/tests/integration` runs against a real Virchow deployment. We cannot
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of Virchow
...
to logs via the `backend/log/<service_name>_debug.log` file. All Virchow services (api_server, web_server, celery_X)
...
**Always raise `VirchowError` from `virchow.error_handling.exceptions` instead of `HTTPException`.
Never hardcode status codes or use `starlette.status` / `fastapi.status` constants directly.**

A global FastAPI exception handler converts `VirchowError` into a JSON response with the standard
...
raise VirchowError(VirchowErrorCode.BAD_GATEWAY, detail, status_code_override=upstream_status)
...
Available error codes are defined in `backend/virchow/error_handling/error_codes.py`. If a new error
...
```python
raise VirchowError(VirchowErrorCode.BAD_GATEWAY, detail, status_code_override=e.response.status_code)
```
